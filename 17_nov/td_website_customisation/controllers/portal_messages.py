...
class PortalMessagesExtended(CustomerPortalBlogsController):
    ...
    def myMessagesListView(self, page=1, **kw):
        ...
        # Sort by unread count, then message count
    ...
    def portal_blog_edit_profile(self, blog_id, **kwargs):
        ...
        # Get selected categories (categories that have at least one selected tag)
...
class CustomerPortalBlogsInherit(CustomerPortal):
    ...
    def _prepare_home_portal_values(self, counters):
        ...
        if 'message_count' in counters:  # ONLY check for message_count
            ...
            # ONLY set message_count for Odoo's counter system
            values['message_count'] = total_msg_count
            # Pass unread_count separately (not as a counter)
            # values['unread_count'] = total_unread_count
            # values['message_unread_count'] = total_unread_count
