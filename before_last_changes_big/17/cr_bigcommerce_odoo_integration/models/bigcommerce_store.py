                ...
                if shipped_items and any(
                    ...
                    try:
                        ...
                        for picking in sale_order.picking_ids:
                            _logger.info(
                                f"[BigCommerce] Starting picking processing: {picking.name} (Initial state: {picking.state})")

                            if picking.state == "draft":
                                picking.action_confirm()
                                _logger.info(f"[BigCommerce] Picking {picking.name} confirmed (state: {picking.state})")

                            if picking.state == "confirmed":
                                picking.action_assign()
                                _logger.info(f"[BigCommerce] Picking {picking.name} assigned (state: {picking.state})")

                            # for move in picking.move_ids_without_package:
                            #     if not move.quantity_done:
                            #         move.quantity_done = move.product_uom_qty
                            #         _logger.info(
                            #             f"[BigCommerce] Move {move.name}: quantity_done set to {move.quantity_done} (product: {move.product_id.display_name})"
                            #         )

                            pre_validate_state = picking.state
                            result = picking.button_validate()
                            _logger.info(
                                f"[BigCommerce] Called button_validate() on {picking.name} (State before: {pre_validate_state}, after: {picking.state})")
            ...
            if picking:
                ...
                if shipped_items and any(
                    ...
                    try:
                        ...
                        #         move.quantity = move.product_uom_qty
                        #
                        # _logger.info(f'1 - picking.state {picking.state}')
                        # if picking.state == "draft":
                        #     _logger.info(f'1 ')
                        #     picking.action_confirm()
                        # _logger.info(f'2 - picking.state {picking.state}')
                        # if picking.state == "confirmed":
                        #     _logger.info(f'2 ')
                        #     picking.action_assign()
                        # _logger.info(f'3 - picking.state {picking.state}')
                        # picking.button_validate()
                        # _logger.info(f'4 - picking.state {picking.state}')
                        # log_message = f"Created and validated delivery for order {order_data['id']} from location {location.name if location else 'default'} with tracking {tracking_number or 'none'}"
                        # _logger.info(log_message)
