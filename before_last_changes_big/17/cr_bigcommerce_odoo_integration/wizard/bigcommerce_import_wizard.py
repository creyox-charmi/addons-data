...
class BigCommerceImportWizard(models.TransientModel):
    ...
    store_id = fields.Many2one(
        ...
        string="Store",
        required=True,
        default=lambda self: self.env.context.get("default_store_id"),
    )
    ...
    import_data_type = fields.Selection(
        selection=IMPORT_DATA_TYPES, string="Import Data Type", required=True
    )
    ...
    def action_import(self):
        ...
        selected_label = dict(self.IMPORT_DATA_TYPES).get(
            self.import_data_type, "Selected Data"
        )
