...
class SaleOrder(models.Model):
    ...
    bigcommerce_order_status_id = fields.Many2one(
        "bigcommerce.order.status", string="BigCommerce Order Status"
    )
    ...
    bigcommerce_shipping_cost_ex_tax = fields.Float(
        "Shipping Cost Excl. Tax", readonly=True
    )
    bigcommerce_shipping_cost_inc_tax = fields.Float(
        "Shipping Cost Incl. Tax", readonly=True
    )
    ...
    def action_get_bigcommerce_order_status(self):
        for order in self:
            ...
            if response.status_code != 200:
                raise UserError(
                    _("Failed to fetch order status from BigCommerce: %s")
                    % response.text
                )
    ...
    def action_update_bigcommerce_order(self):
        for order in self:
            ...
            if response.status_code != 200:
                raise UserError(
                    _("Failed to fetch BigCommerce order products:\n%s") % response.text
                )
            ...
            for line in order.order_line:
                ...
                key = (
                    line.product_id.bigcommerce_product_id,
                    tuple(sorted(variant_opts)),
                )
                bc_product = bc_products_map.get(key)

                if bc_product:
                    qty_differs = bc_product["quantity"] != int(line.product_uom_qty)
                    price_differs = float(bc_product.get("price_inc_tax", 0)) != float(
                        line.price_unit
                    )
                    ...
                    if qty_differs or price_differs:
                        ...
                        if variant_opts:
                            product_data["product_options"] = [
                                {"id": o, "value": v} for o, v in variant_opts
                            ]
            ...
            payload = {
                ...
                "order_is_digital": all(
                    l.product_id.type == "service" for l in order.order_line
                ),
                "items_total": len(order.order_line),
                "items_shipped": sum(l.qty_delivered for l in order.order_line),
                "payment_method": order.payment_term_id.name
            ...
            if resp.status_code != 200:
                raise UserError(
                    _("Failed to update order in BigCommerce:\n%s - %s")
    ...
    def action_import_bigcommerce_order(self):
        for order in self:
            ...
            if response.status_code != 200:
                raise UserError(
                    _("Failed to fetch BigCommerce order:\n%s") % response.text
                )
            ...
            order.write(
                {
                    "partner_id": self.env[
                        "res.partner"
                    ]._find_or_create_from_bigcommerce(bc_order["billing_address"]),
                    "amount_total": bc_order["total_inc_tax"],
                    "amount_untaxed": bc_order["total_ex_tax"],
                    "note": bc_order.get("staff_notes", ""),
                    "bigcommerce_order_status_id": self.env["bigcommerce.order.status"]
                    .search(
                        [("external_status_id", "=", bc_order["status_id"])], limit=1
                    )
                    .id,
                }
            )
            ...
            if resp_products.status_code != 200:
                raise UserError(
                    _("Failed to fetch products for order:\n%s") % resp_products.text
                )

            for p in resp_products.json():
                product = self.env["product.product"].search(
                    [("bigcommerce_product_id", "=", p["product_id"])], limit=1
                )
                if not product:
                    product = self.env["product.product"].create(
                        {
                            ...
                            "lst_price": p.get("price_inc_tax")
                            or p.get("price_ex_tax"),
                        }
                    )
                ...
                order.order_line.create(
                    {
                        ...
                        "price_unit": float(
                            p.get("price_inc_tax") or p.get("price_ex_tax") or 0
                        ),
                    }
                )
    ...
    def action_bulk_update_bigcommerce_order(self):
        for order in self:
            try:
                ...
                _logger.exception(
                    "Failed to update BigCommerce order %s: %s", order.name, str(e)
                )
                raise UserError(
                    _("Failed to update BigCommerce order %s: %s")
                    % (order.name, str(e))
                )
    ...
    def action_bulk_import_bigcommerce_order(self):
        for order in self:
            try:
                ...
                _logger.exception(
                    "Failed to import BigCommerce order %s: %s", order.name, str(e)
                )
                raise UserError(
                    _("Failed to import BigCommerce order %s: %s")
                    % (order.name, str(e))
                )
    ...
    #         _logger.warning(
    #             "No delivery product configured for carrier '%s' on order '%s'",
    #             carrier.name,
    #             self.name,
    #         )
    #         return False
    ...
    #         # Safe to remove and recreate delivery line
    #         if delivery_lines:
    #             _logger.info(
    #                 "Removing existing delivery lines for draft order '%s'", self.name
    #             )
    ...
    #         )
    #         _logger.info(
    #             "Created new delivery line for draft order '%s' with cost %.2f",
    #             self.name,
    #             amount,
    #         )
    #     else:
    #         # Order is confirmed — cannot unlink delivery lines
    #         if delivery_lines:
    #             _logger.info(
    #                 "Updating existing delivery line for confirmed order '%s'",
    #                 self.name,
    #             )
    ...
    #         else:
    #             _logger.info(
    #                 "Creating delivery line for confirmed order '%s'", self.name
    #             )
    #             self.env["sale.order.line"].create(
    #                 {
    #                     "order_id": self.id,
    ...
    #     _logger.info(
    #         "Set delivery cost %.2f on order '%s' for carrier '%s'",
    #         amount,
    #         self.name,
    #         carrier.name,
    #     )
    ...
    def _set_delivery_cost(self, carrier, amount):
        ...
        zip_code = shipping_partner.zip

        # Use BigCommerce shipping tax with class_id = 0 (special logic in _get_applicable_tax)
        tax_class = '2' if carrier.product_id.type == 'service' else '0'
        tax = self.bigcommerce_store_id._get_applicable_tax(
        ...
        # taxes = tax and [tax.id] or []

        if not tax:
            _logger.warning(
                "[BigCommerce] No applicable shipping tax found for country=%s, state=%s, zip=%s",
                country_id,
                state_id,
                zip_code,
            )
        ...
        if self.state in ["draft", "sent"]:
            if delivery_lines:
                _logger.info("Removing existing delivery lines for draft order '%s'", self.name)
                delivery_lines.unlink()
            ...
            self.env["sale.order.line"].create(
                {
                    ...
                    "product_uom_qty": 1,
                    "is_delivery": True,
                    "tax_id": [(6, 0, taxes.ids if hasattr(taxes, 'ids') else [taxes.id])],
                }
            )
            _logger.info(
                "Created new delivery line for draft order '%s' with cost %.2f",
            ...
            if delivery_lines:
                _logger.info("Updating existing delivery line for confirmed order '%s'", self.name)
                delivery_lines.write(
                    {
                        "price_unit": amount,
                        "product_uom_qty": 1,
                ...
                _logger.info("Creating delivery line for confirmed order '%s'", self.name)
                self.env["sale.order.line"].create(
                    {
                        "order_id": self.id,
                        "product_id": carrier.product_id.id,
    ...
    def _action_create_delivery(self):
        ...
        delivery_wizard = self.env["stock.immediate.transfer"].create(
            {"pick_to_backorder": False, "picking_ids": [(6, 0, [])]}
        )
