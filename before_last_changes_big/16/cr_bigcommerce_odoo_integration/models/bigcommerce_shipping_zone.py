    ...
    def update_zone_from_bigcommerce(self):
        for record in self:
            ...
            if response.status_code != 200:
                raise UserError(
                    _("Failed to fetch shipping zone. Response: %s") % response.text
                )
            ...
            record.write(
                {
                    "name": data.get("name"),
                    "type": data.get("type"),
                    "free_shipping_enabled": data.get("free_shipping", {}).get(
                        "enabled"
                    ),
                    "free_shipping_minimum_sub_total": float(
                        data.get("free_shipping", {}).get("minimum_sub_total", 0.0)
                    ),
                    "exclude_fixed_shipping_products": data.get(
                        "free_shipping", {}
                    ).get("exclude_fixed_shipping_products"),
                    "display_separately": data.get("handling_fees", {}).get(
                        "display_separately"
                    ),
                    "fixed_surcharge": float(
                        data.get("handling_fees", {}).get("fixed_surcharge", 0.0)
                    ),
            ...
            return {
                ...
                "params": {
                    ...
                    "message": _("Successfully updated the shipping zone '%s'.")
                    % record.name,
                    "type": "success",
                    "sticky": False,
                },
    ...
    def export_zone_to_bigcommerce(self):
        for record in self:
            ...
            payload = {
                ...
                "locations": [
                    {
                        "id": int(record.location_id.bc_location_id),
            ...
            if response.status_code != 200:
                raise UserError(
                    _("Failed to update zone in BigCommerce. Response: %s")
                    % response.text
                )
            ...
            return {
                ...
                "params": {
                    ...
                    "message": _("Zone '%s' successfully updated in BigCommerce.")
                    % record.name,
                    "type": "success",
                    "sticky": False,
                },
            ...
            if response.status_code == 204:
                ...
                return {
                    ...
                    "params": {
                        ...
                        "message": _(
                            "Shipping Zone deleted successfully from BigCommerce and Odoo."
                        ),
                        "type": "success",
                        "sticky": False,
                    },
                ...
                raise UserError(
                    _("Failed to delete shipping zone. Response: %s") % response.text
                )
        return None
