...
class BigCommerceStore(models.Model):
    ...
    #                             "=",
    #                             store.id,
    #                         ),  # optional if you store brand per store
    #                     ],
    #                     limit=1,
    #                 )
    #
    ...
    #                         "bigcommerce_product_tax_code": item.get(
    #                             "product_tax_code"
    #                         ),
    #                         "bigcommerce_calculated_price": item.get(
    #                             "calculated_price"
    #                         ),
    ...
    #                         "bigcommerce_meta_description": item.get(
    #                             "meta_description"
    #                         ),
    ...
    def import_bigcommerce_products(self):
        ...
        fail_count = 0
        for store in self:
            _logger.info(f"[BigCommerce] Starting product import for store: {store.name} (ID: {store.id})")
            ...
            url = f"https://api.bigcommerce.com/stores/{store.store_hash}/v3/catalog/products"
            _logger.info(f"[BigCommerce] Sending GET request to: {url}")
            response = requests.get(url, headers=headers)

            _logger.info(f"[BigCommerce] Received response: {response.status_code}")
            if response.status_code == 200:
                data = response.json().get("data", [])
                _logger.info(f"[BigCommerce] Number of products received: {len(data)}")
                ...
                _logger.info(f"[BigCommerce] Shop All category ID: {shop_all_category_id}")

                for item in data:
                    _logger.info(f"[BigCommerce] Processing product ID: {item.get('id')} - {item.get('name')}")
                    existing_product = self.env["product.template"].search(
                        [("bigcommerce_product_id", "=", item.get("id")),("bigcommerce_store_id", "=", store.id),], limit=1
                    )

                    if existing_product:
                        skipped_count += 1
                        _logger.info(f"[BigCommerce] Product already exists in Odoo, skipping: {existing_product.name}")
                        continue
                    ...
                    _logger.info(f"[BigCommerce] Category IDs for product: {category_ids}")

                    brand_id = item.get("brand_id")
                    _logger.info(f"[BigCommerce] Brand ID: {brand_id}")
                    ...
                    _logger.info(f"[BigCommerce] Brand record found: {brand_rec.name if brand_rec else 'None'}")
                    ...
                    _logger.info(f"[BigCommerce] Creating product: {product_vals['name']}")
                    product = self.env["product.template"].create(product_vals)

                    _logger.info(f"[BigCommerce] Importing images and variants for product: {product.name}")
                    product.import_product_image()
                    product.import_bigcommerce_variants()
                    created_count += 1
                    _logger.info(f"[BigCommerce] Product created: {product.name}")

                _logger.info(f"[BigCommerce] Finished importing {created_count} products.")
                self.env["cr.data.processing.log"].sudo()._log_data_processing(
                    cr_shop_id=self.id,
                    cr_message=(
                        f"Product import complete: {created_count} created, "
                ...
                return {
                    ...
                    "params": {
                        ...
                        "title": _("Import Successfully"),
                        "message": _(
                            "Products Imported: %d created, %d skipped."
                ...
                self.env["cr.data.processing.log"].sudo()._log_data_processing(
                    cr_shop_id=self.id,
                    cr_message="Failed to fetch products from BigCommerce.",
                    record_count=fail_count,
                    status="failure",
                    timespan=str(fields.Datetime.now()),
                    initiated_at=str(fields.Datetime.now()),
    ...
    #                     [("bigcommerce_customer_id", "=", customer["id"])], limit=1
    #                 )
    #                 vals = {
    #                     "bigcommerce_store_id": store.id,
    #                     "company_id": store.company_id.id,
    ...
    #                     "bigcommerce_registration_ip": customer.get(
    #                         "registration_ip_address"
    #                     ),
    #                     "bigcommerce_tax_exempt_category": customer.get(
    #                         "tax_exempt_category"
    #                     ),
    #                     "bigcommerce_date_created": parse_datetime(
    #                         customer.get("date_created")
    #                     ),
    #                     "bigcommerce_date_modified": parse_datetime(
    #                         customer.get("date_modified")
    #                     ),
    #                     "bigcommerce_accepts_review_emails": customer.get(
    #                         "accepts_product_review_abandoned_cart_emails"
    #                     ),
    #                     "bigcommerce_origin_channel_id": customer.get(
    #                         "origin_channel_id"
    #                     ),
    #                 }
    #
        ...
        for store in self:
            ...
            try:
                ...
                if created_count:
                    self.env["cr.data.processing.log"].sudo()._log_data_processing(
                        cr_shop_id=self.id,
                        cr_message="Successfully created new customer(s).",
                        record_count=created_count,
                        status="success",
                        timespan=str(fields.Datetime.now()),
                        initiated_at=str(fields.Datetime.now()),
                ...
                # Log updated (skipped) customers
                if updated_count:
                    self.env["cr.data.processing.log"].sudo()._log_data_processing(
                        cr_shop_id=self.id,
                        cr_message="Successfully updated existing customer(s).",
                ...
                return {
                    ...
                    "params": {
                        ...
                        "message": _(
                            "Customers Imported: %d created, %d updated." % (created_count, updated_count)
                        ),
                        "sticky": False,
                        "next": {"type": "ir.actions.client", "tag": "soft_reload"},
                    },
    ...
    #                     [("bigcommerce_customer_id", "=", customer_id)], limit=1
    #                 )
    #
    #                 if not partner:
    #                     continue
            ...
            try:
                ...
                return {
                    ...
                    "params": {
                        "type": "success",
                        "title": _("Addresses Imported"),
                        "message": _("Sucessfully fetched %d customers address."% (count)),
                        "sticky": False,
                    },
                }
    ...
    #             "message": _(
    #                 "Order status records were successfully imported from BigCommerce."
    #             ),
    #             "sticky": False,
    #         },
    #     }
    ...
    #                     "is_customized_url": brand.get("custom_url", {}).get(
    #                         "is_customized", False
    #                     ),
    #                     "store_id": store.id,
    #                 }
    ...
    def action_export_products_to_bigcommerce(self):
        ...
        products = self.env["product.template"].search(
            [("bigcommerce_product_id", "=", None), ("detailed_type", "=", "product")]
        )
    ...
    def action_delete_product_inventory_updated_webhook(self):
        ...
        self.ensure_one()
        webhook = self.webhook_ids.filtered(
            lambda w: w.scope == "store/product/inventory/updated"
        )
        if not webhook:
            self.env["cr.data.processing.log"].sudo()._log_data_processing(
                cr_shop_id=self.id,
    ...
    def action_delete_sku_inventory_updated_webhook(self):
        ...
        self.ensure_one()
        webhook = self.webhook_ids.filtered(
            lambda w: w.scope == "store/sku/inventory/updated"
        )
        if not webhook:
            self.env["cr.data.processing.log"].sudo()._log_data_processing(
                cr_shop_id=self.id,
    ...
    def _get_applicable_tax(self, country_id, state_id, zip_code, tax_class='0'):
        _logger.info(
            f"[BigCommerce] Starting tax search for Country ID: {country_id}, State ID: {state_id}, Zip: {zip_code}, Tax Class: {tax_class}")
        ...
        def _get_tax_from_fp(fp):
            if not fp:
                _logger.info(f"[BigCommerce] No fiscal position provided.")
                return None
            _logger.info(f"[BigCommerce] Getting tax from Fiscal Position: {fp.name} (ID: {fp.id})")

            tax_lines = FiscalPositionTax.search([
                ('position_id', '=', fp.id),
            ])
            _logger.info(f"[BigCommerce] Found {len(tax_lines)} fiscal position tax lines.")

            taxes = tax_lines.mapped('tax_dest_id')
            _logger.info(f"[BigCommerce] Found {len(taxes)} tax_dest_id from tax lines.")

            taxes = taxes.filtered(
                lambda t: t.company_id.id == self.env.company.id and t.bigcommerce_store_id.id == self.id
            )
            _logger.info(f"[BigCommerce] Filtered taxes matching store/company: {len(taxes)}")
            target_taxes = taxes.filtered(lambda t: str(t.bigcommerce_tax_class_id or '0') == str(tax_class))
            _logger.info(f"[BigCommerce] Filtered taxes with bigcommerce_tax_class_id={tax_class}: {len(target_taxes)}")

            if target_taxes:
                _logger.info(f"[BigCommerce] Returning tax (by class match): {target_taxes[0].name}")
                return target_taxes[0]
            elif taxes:
                _logger.info(f"[BigCommerce] Returning fallback tax (first in list): {taxes[0].name}")
                return taxes[0]

            _logger.info("[BigCommerce] No applicable tax found in fiscal position.")
            return None

        # Priority 1: Match via postal code
        _logger.info("[BigCommerce] Checking FiscalPositionPostal for country & postal code match...")
        postal_matches = FiscalPositionPostal.search([
            ("country_id", "=", country_id),
            ("postal_code", "=", zip_code),
        ])
        _logger.info(f"[BigCommerce] Found {len(postal_matches)} postal matches.")
        ...
        _logger.info(f"[BigCommerce] Filtered {len(fpos_from_postal)} fiscal positions from postal code.")

        if fpos_from_postal:
            tax = _get_tax_from_fp(fpos_from_postal[0])
            if tax:
                _logger.info(f"[BigCommerce] Tax found from postal-based fiscal position: {tax.name}")
                return tax

        # Priority 2: country + state
        _logger.info("[BigCommerce] Checking fiscal position by country + state...")
        ...
        if tax:
            _logger.info(f"[BigCommerce] Tax found from country + state fiscal position: {tax.name}")
            return tax

        # Priority 3: country only
        _logger.info("[BigCommerce] Checking fiscal position by country only...")
        fpos = FiscalPosition.search([
            ("country_id", "=", country_id),
            ("bigcommerce_store_id", "=", self.id),
            ("company_id", "=", self.env.company.id),
        ...
        tax = _get_tax_from_fp(fpos)
        if tax:
            _logger.info(f"[BigCommerce] Tax found from country-only fiscal position: {tax.name}")
            return tax

        # Priority 4: fallback (no country)
        _logger.info("[BigCommerce] Checking global fallback fiscal positions (no country)...")
        fpos = FiscalPosition.search([
            ("country_id", "=", False),
            ("bigcommerce_store_id", "=", self.id),
            ("company_id", "=", self.env.company.id),
        ])
        _logger.info(f"[BigCommerce] Found {len(fpos)} global fallback fiscal positions.")

        for fp in fpos:
            tax = _get_tax_from_fp(fp)
            if tax:
                _logger.info(f"[BigCommerce] Tax found from fallback fiscal position: {tax.name}")
                return tax

        _logger.info("[BigCommerce] No applicable tax found. Returning None.")
        return None
    ...
    def _process_bigcommerce_order(self, order_data, headers):
        ...
        for prod in prod_response.json():
            cr_product = self.get_product(prod)
            _logger.info(f'cr_product {cr_product}')
            product = (
                self.env["product.product"].browse(cr_product.get("id"))
                if cr_product.get("id")
                else None
            ...
            if product:
                ...
                if response.status_code == 200:
                    ...
                    _logger.info("Tax Class ID: %s", tax_class_id)
                else:
        ...
        _logger.info(f'final_shipping_cost : {final_shipping_cost}')
        if carrier and final_shipping_cost > 0:
            if sale_order._set_delivery_cost(carrier, final_shipping_cost):
                _logger.info(
                    "Set delivery cost %s for order %s with carrier %s",
        ...
        if order_data.get("status") == "Shipped":
            ...
            if picking:
                    ...
                    for item in shipped_items:
                        ...
                        if product_data:
                            ...
                            if move:
                                ...
                                move.quantity_done = item["quantity"]
                else:
                    _logger.info(
                        "No shipped items for order %s, skipping picking validation",
                        order_data["id"],
                ...
                if shipped_items and any(
                    move.quantity_done > 0 for move in picking.move_ids_without_package
                ):
                    try:
                        for move in picking.move_ids_without_package:
                            if move.product_uom_qty != move.quantity_done:
                                ...
                                _logger.info(
                                    f"[BigCommerce]move.quantity_done: {move.quantity_done} (move.product_uom_qty: {move.product_uom_qty})")


                        for picking in sale_order.picking_ids:
                            _logger.info(f"[BigCommerce] Preparing picking: {picking.name} (state: {picking.state})")


                            # Confirm + Assign if needed
                            if picking.state == 'draft':
                                picking.action_confirm()
        ...
        for store in self:
            ...
            for order in orders:
                ...
                if data:
                    ...
                    result = self._process_bigcommerce_order(order, headers)
        ...
        for prod in prod_response.json():
            ...
            if not product:
                product = (
                    ...
                    .sudo()
                    .search(
                        [("bigcommerce_product_id", "=", prod["product_id"])], limit=1
                    )
                )
    ...
    #         _logger.info(f"[BigCommerce] Processing line: {line_data}")
    #         matching_line = existing_lines.filtered(
    #             lambda l: l.product_id.bigcommerce_product_id
    #             == line_data["bigcommerce_product_id"]
    #             and l.product_id.bigcommerce_product_attribute_id
    #             == line_data["bigcommerce_product_attribute_id"]
    #         )
    #         if matching_line:
    #             _logger.info(
    #                 f"[BigCommerce] Updating existing line: {matching_line[0].id}"
    #             )
    ...
    #         else:
    #             _logger.info(
    #                 f"[BigCommerce] Adding new order line for product ID: {line_data['product_id']}"
    #             )
    ...
    #             _logger.info(
    #                 f"[BigCommerce] Removed sale order line {line.id} from order {order_id}"
    #             )
    #
    #     sale_order_vals["order_line"] = updated_lines
    #     _logger.info(
    #         f"[BigCommerce] Final sale_order_vals for order {order_id}: {sale_order_vals}"
    #     )
    #
    #     # Write sale order
    #     try:
    ...
    #                     picking.button_validate()
    #                     _logger.info(
    #                         f"[BigCommerce] Picking validated for order {order_id}"
    #                     )
    #                 except Exception as e:
    #                     _logger.error(
    ...
    def _update_bigcommerce_order(self, order_data, headers, sale_order):
        ...
        try:
            ...
            if odoo_state == "sale":
                if sale_order.state != "sale":
                    sale_order.action_confirm()
                for data in sale_order.picking_ids:
                    _logger.info(f'data.state : {data.state}')
                    if data.state == "confirmed":
                        for move in data.move_ids_without_package:
                            valid_quants = move.product_id.stock_quant_ids.filtered(
                                lambda q: q.location_id.usage == "internal"
        ...
        # Delivery cost
        if carrier and final_shipping_cost:
            _logger.info(
                f"[BigCommerce] Trying to set delivery cost: {final_shipping_cost} with carrier {carrier.name}"
            )
            if sale_order._set_delivery_cost(carrier, final_shipping_cost):
    ...
    def _sync_tax_classes(self):
        ...
        try:
            ...
            error_msg = (
                f"Failed to fetch tax classes for store {self.store_hash}: {str(e)}"
            )
            _logger.error(error_msg)
            self.env["cr.data.processing.log"]._log_data_processing(
                cr_shop_id=self.id,
        ...
        for tc in tax_classes:
            ...
            if not tax:
                ...
                _logger.info(
                    "Created tax %s for BigCommerce tax class %s (store %s)",
                    tax_name,
                    tc["id"],
                    self.store_hash,
                )
    ...
    #     except requests.exceptions.RequestException as e:
    #         error_msg = (
    #             f"Failed to fetch tax zones for store {self.store_hash}: {str(e)}"
    #         )
    #         _logger.error(error_msg)
    #         self.env["cr.data.processing.log"]._log_data_processing(
    #             cr_shop_id=self.id,
    ...
    #     except requests.exceptions.RequestException as e:
    #         error_msg = (
    #             f"Failed to fetch tax rates for store {self.store_hash}: {str(e)}"
    #         )
    #         _logger.error(error_msg)
    #         self.env["cr.data.processing.log"]._log_data_processing(
    #             cr_shop_id=self.id,
    ...
    #         )
    #         _logger.error(
    #             "Failed to fetch tax rates for store %s: %s. Response: %s",
    #             self.store_hash,
    #             str(e),
    #             getattr(rates_response, "text", "No response"),
    #         )
    ...
    #                 country = self.env["res.country"].search(
    #                     [("code", "=", loc["country_code"])], limit=1
    #                 )
    #                 if country:
    #                     country_id = country.id
    #             if loc.get("subdivision_codes"):
    #                 states = self.env["res.country.state"].search(
    #                     [("code", "in", loc["subdivision_codes"])]
    #                 )
    #                 if states:
    ...
    #             )
    #             _logger.warning(
    #                 "No tax rates found for zone %s (%s), using 0%% rates (store %s)",
    #                 zone_id,
    #                 zone_name,
    #                 self.store_hash,
    #             )
    ...
    #                             [
    #                                 (
    #                                     "bigcommerce_tax_class_id",
    #                                     "=",
    #                                     str(class_rate["tax_class_id"]),
    #                                 ),
    #                                 ("bigcommerce_store_id", "=", self.id),
    #                                 ("company_id", "=", self.env.company.id),
    #                             ],
    ...
    #                         tax_mappings.append(
    #                             {"tax_src_id": tax.id, "tax_dest_id": tax.id}
    #                         )
    #
    ...
    #             created_positions += 1
    #             _logger.info(
    #                 "Created fiscal position %s for BigCommerce zone %s (store %s)",
    #                 zone_name,
    #                 zone_id,
    #                 self.store_hash,
    #             )
    ...
    def _prepare_sale_order_vals(
        ...
        country = self.env["res.country"].search(
            [("code", "=", shipping_addresses[0].get("country_iso2"))], limit=1
        )
        _logger.info(f"country {country}")
        state = self.env["res.country.state"].search(
            [("code", "=", shipping_addresses[0].get("state"))], limit=1
        )
    ...
    #             )
    #             raise UserError(
    #                 _("Failed to sync tax classes for store %s.") % self.store_hash
    #             )
    #
    #         self.env["cr.data.processing.log"]._log_data_processing(
    #             cr_shop_id=self.id,
    ...
    #             )
    #             raise UserError(
    #                 _("Failed to sync products for store %s.") % self.store_hash
    #             )
    #
    #         self.env["cr.data.processing.log"]._log_data_processing(
    #             cr_shop_id=self.id,
    ...
    #         message = (
    #             _("Taxes and products synced successfully for store %s.")
    #             % self.store_hash
    #         )
    #         if skipped_zones:
    ...
    def action_open_shipped_orders_for_store(self):
        ...
        return {
            ...
            "view_mode": "tree,form",
            "target": "current",
            "domain": [("bigcommerce_store_id", "=", self.id), ("state", "=", "done")],
            "context": {"default_bigcommerce_store_id": self.id},
        }
