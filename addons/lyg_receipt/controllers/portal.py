...
class Portal(CustomerPortal):
    ...
    def _prepare_my_receipt_values(self, page, date_begin, date_end, sortby,
        ...
        # Replace expression.AND with api.domain
        domain = domain + self.common_domain_for_receipt()
    ...
    @http.route(['/my/receipt/<int:receipt_id>'], type='http', auth="public",
                website=True)
    ...
    def _create_transaction(self, *args, payment_option_id=None, provider_reference=None, **kwargs):
        """Create a transaction and assign the partner from the first
         invoice or sale order."""
        res = super()._create_transaction(*args,
