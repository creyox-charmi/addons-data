...
class AccountPayment(models.Model):
    ...
    #     for rec in self:
    #         if rec.partner_type == 'customer' and self.env.context.get(
    ...
    def write(self, vals):
        _logger.info(
            "Writing AccountPayment ID(s): %s with values: %s",
            self.ids, vals
        )
        ...
        for rec in self:
            if rec.partner_type == 'customer' and self.env.context.get(
                    'customer_payment') and 'is_deposited' not in vals:
                _logger.warning(
                    "Blocked customer payment modification. Payment ID: %s",
                    rec.id
                )
                raise UserError(
                    _("""You can not create or edit customer payment, Please create receipt for this payment..!!"""))
    ...
    def get_reconcile_ids(self, payments):
        ...
        _logger.info(
            "Reconcile result for payment %s: %s",
            payments.id, query_res
        )
    ...
    # def create_receipt(self, payments):
    #     _logger.info(
    #         "Starting receipt creation for payments: %s",
    #         payments.ids
    #     )
    #
    #     payments._compute_stat_buttons_from_reconciliation()
    ...
    #
    #     _logger.info(
    #         "Reconciliation data for payments %s: %s",
    #         payments.ids, res
    #     )
    #
    #     receipt_id = self.env['lyg.account.receipt'].with_context(
    ...
    #
    #     _logger.info(
    #         "Receipt created. Receipt ID: %s for payments: %s",
    #         receipt_id.id, payments.ids
    #     )
    #
    #     payments.write({'receipt_id': receipt_id.id, 'means_of_payment': '3'})
    #     receipt_id.write({'state': 'post'})
...
class PaymentTransaction(models.Model):
    ...
    #     _logger.info("=== IN PAYMENT TRANSACTION _create_payment ===")
    ...
    #     _logger.info(f"=== self.env.context.get('lastcall') === {self.env.context.get('lastcall')}")
    #     if self.env.context and not self.env.context.get('lastcall'):
    #         _logger.info("=== FIrst IF ===")
    #         if res.partner_type == 'customer' and res.company_id.account_fiscal_country_id.code == 'IN':
    ...
    def _create_payment(self, **extra_create_values):
        _logger.info(
            "Creating payment from transaction ID: %s",
            self.id
        )
        ...
        _logger.info(
            "Payment created. Payment ID: %s, Partner type: %s",
            res.id, res.partner_type
        )

        if self.env.context and not self.env.context.get('lastcall'):
            if res.partner_type == 'customer' and \
                    res.company_id.account_fiscal_country_id.code == 'IN':
                _logger.info(
                    "Creating receipt for payment ID: %s",
                    res.id
                )
                ...
                _logger.info(
                    "Receipt %s created for payment %s",
                    receipt.id, res.id
                )
