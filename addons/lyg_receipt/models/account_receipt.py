...
class AccountReceipt(models.Model):
    ...
    payment_label = fields.Char(
        ...
        compute="_compute_payment_label",
        store=False
    )
    ...
    def _compute_payment_label(self):
        for rec in self:
            if rec.payment_count == 1:
                rec.payment_label = "Payment"
            elif rec.payment_count > 1:
                rec.payment_label = "Payments"
            else:
                rec.payment_label = ""
    ...
    @api.constrains('validity_date', 'means_of_payment')
    def _check_validity_date(self):
        invalid_lines = self.filtered(
            lambda r: r.means_of_payment != '2'
                      and r.validity_date
                      and r.validity_date < fields.Date.today()
        )
        if invalid_lines:
            raise ValidationError(_("Validity date cannot be older than today for non-check payments."))
    ...
    def action_post_receipt(self):
        ...
        for receipt in self:
            partner = receipt.partner_id
            if partner.country_id and partner.country_id.code != "IL":
                if not partner.attachments:
                    raise ValidationError(
                        _("עבור תשלום במזומן ללקוח מחול, יש לצרף מסמך מזהה")
                    )
...
class AccountReceiptLine(models.Model):
    ...
    def create(self, vals_list):
        """Overrides the create method to automatically generate copied
            receipt lines if `copy_receipt_line` is enabled and `qty_copy` is specified."""
    ...
    def _generate_copy_receipt_lines(self):
        ...
        and increments the validity date for each additional line."""
        new_lines = []
        ...
        for i in range(1, int(self.qty_copy)):
            ...
            new_vals = {
                ...
                'type': self.type,
                'amount': self.amount,
                'branch': self.branch if self.branch else False,
                'credit_account_no': self.credit_account_no,
                'bank_id': self.bank_id if self.bank_id else False,
                'validity_date': base_date + relativedelta(months=i),
                'voucher_check_no': str(base_voucher_check_no + i),
    ...
    def convert_invoice_amount_to_receipt_currency(self):
        """
        HT02041

        Convert the residual amount of the linked invoice into the receipt's currency.

        Logic:
        - If the receipt currency and invoice currency are the same,
          return the invoice residual amount directly.
        - If the receipt currency matches the company currency AND the invoice
          residual equals the invoice total, return the invoice tax total amount.
        - Otherwise, convert the residual amount from the invoice currency to
          the receipt currency using the invoice date for conversion.
        :return: Residual amount of the invoice in the receipt's currency (float).
        """
        invoice_id = self.invoice_id
        if self.currency_id.id == invoice_id.currency_id.id:
            amount_residual = invoice_id.amount_residual
        elif (invoice_id.company_id.currency_id.id == self.currency_id.id and
              invoice_id.amount_residual == invoice_id.amount_total):
            amount_residual = invoice_id.tax_totals.get('total_amount')
        else:
            amount_residual = invoice_id.currency_id._convert(
                from_amount=invoice_id.amount_residual,
