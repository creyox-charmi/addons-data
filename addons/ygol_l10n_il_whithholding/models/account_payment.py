...
class AccountPayment(models.Model):
    ...
    def _compute_amount_after_withholding(self):
        for payment in self:
            if hasattr(payment, 'hmk_withholding_payment') and payment.hmk_withholding_payment:
                payment.amount_after_withholding = payment.amount
            else:
                payment.amount_after_withholding = payment.amount - payment.withtholding_amount

    def update_amount_after_withholding_cron(self):
        """Cron job to update amount_after_withholding for all records."""
        payments = self.env['account.payment'].search([])
        for payment in payments:
            if payment.hmk_withholding_payment:
                payment.amount_after_withholding = payment.amount
            else:
            ...
            if company_id.id == self.env.company.id:
                ...
                if withholding:
                    ...
                    for i, pay in enumerate(payments):
                        ...
                        if hasattr(self, 'receipt_id') and pay.receipt_id and pay.withholding_payment:
    ...
    def get_withholding_condition(self):
        ...
        for rec in self:
            ...
            elif hasattr(self, 'receipt_id') and rec.receipt_id and rec.withholding_payment:
                withholding = True
        ...
        if self.company_id.withholding_tax_process and withholding:
            ...
            if self.partner_type == 'supplier':
                ...
                if self.payment_type == 'inbound':
                    ...
                    # Send money.
                    liquidity_amount_currency = - (self.amount) if hasattr(self, 'receipt_id') and self.receipt_id.is_credit_receipt else -(
