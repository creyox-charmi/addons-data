...
class BigCommerceStore(models.Model):
    ...
    @api.onchange('id')
    def _onchange_default_product_category_domain(self):
        if self.id:
            return {
                'domain': {
    ...
    # def import_bigcommerce_products(self):
    #     """Fast import of products with proper transaction handling."""
    #     import time
    #     import concurrent.futures
    #
    #     for store in self:
    #         if not store.access_token or not store.store_hash:
    #             raise UserError("Store Hash and Access Token are required.")
    #
    ...
    #             url = f"https://api.bigcommerce.com/stores/{store.store_hash}/v3/catalog/products?page={page_num}&limit=250"
    #             try:
    ...
    #             except Exception as e:
    #                 _logger.error(f"Error fetching page {page_num}: {e}")
    #             return [], {}
    #
    #         try:
    #             # Step 1: Fetch first page
    #             _logger.info("Fetching first page to get total product count...")
    #             first_page_data, meta = fetch_page(1)
    #             pagination = meta.get("pagination", {})
    #             total = pagination.get("total", 0)
    #             total_pages = pagination.get("total_pages", 1)
    #
    #             _logger.info(f"Starting import of {total} products across {total_pages} pages")
    #
    #             all_products = first_page_data
    #
    #             # Step 2: Fetch remaining pages in parallel (API calls only, no DB operations)
    #             if total_pages > 1:
    ...
    #                             _logger.info(f"Fetched {len(all_products)}/{total} products...")
    #
    #             _logger.info(f"Fetched all {len(all_products)} products in {time.time() - start_time:.2f}s")
    #
    #             # Step 3: Pre-load reference data
    #             shop_all_category_id = self.env["product.category"].sudo().search([
    #                 ("bigcommerce_store_id", "=", store.id),
    #                 ("name", "ilike", "shop all"),
    #             ], limit=1).id
    #
    #             all_bc_ids = [str(p["id"]) for p in all_products]
    #             existing_products = self.env["product.template"].sudo().search([
    #                 ("bigcommerce_product_id", "in", all_bc_ids),
    #                 ("bigcommerce_store_id", "=", store.id),
    #             ])
    #             existing_map = {str(p.bigcommerce_product_id): p.id for p in existing_products}
    #
    #             all_category_ids = []
    #             for item in all_products:
    #                 all_category_ids.extend(item.get("categories", []))
    #
    #             category_map = {}
    #             if all_category_ids:
    #                 categories = self.env["product.category"].sudo().search([
    #                     ("bigcommerce_category_id", "in", list(set(all_category_ids))),
    #                     ("bigcommerce_store_id", "=", store.id),
    #                     ("id", "!=", shop_all_category_id),
    #                 ])
    #                 for cat in categories:
    #                     bc_cat_id = str(cat.bigcommerce_category_id)
    #                     if bc_cat_id not in category_map:
    #                         category_map[bc_cat_id] = []
    #                     category_map[bc_cat_id].append(cat.id)
    #
    #             brand_ids = [p.get("brand_id") for p in all_products if p.get("brand_id")]
    #             brand_map = {}
    #             if brand_ids:
    #                 brands = self.env["bigcommerce.brand"].sudo().search([
    #                     ("brand_id", "in", list(set(brand_ids))),
    #                     ("store_id", "=", store.id),
    #                 ])
    #                 brand_map = {b.brand_id: b.id for b in brands}
    #
    ...
    #
    #             for item in all_products:
    #                 if str(item["id"]) in existing_map:
    #                     skipped_count += 1
    #                     continue
    #
    #                 category_ids = []
    ...
    #                         category_ids.extend(category_map[cat_key])
    #
    #                 if not category_ids:
    #                     category_ids = [store.default_product_category_id.id]
    #
    #                 brand_id = brand_map.get(item.get("brand_id"), False)
    #
    #                 product_vals = {
    #                     "name": item.get("name") or "Unnamed Product",
    #                     "bigcommerce_store_id": store.id,
    #                     "bigcommerce_product_id": str(item.get("id")),
    #                     "default_code": item.get("sku"),
    ...
    #                     "bigcommerce_meta_description": item.get("meta_description"),
    #                     "bigcommerce_view_count": item.get("view_count"),
    #                     "categ_id": category_ids[0] if category_ids else store.default_product_category_id.id,
    #                     "company_id": store.company_id.id,
    #                     "type": "consu",
    #                     "is_storable": True,
    #                 }
    #
    #                 to_create.append(product_vals)
    #
    #             # Step 5: Batch create products
    #             created_products = []
    #             created_count = 0
    #
    #             if to_create:
    #                 _logger.info(f"Creating {len(to_create)} products in batches...")
    #
    #                 batch_size = 100
    #                 for i in range(0, len(to_create), batch_size):
    #                     batch = to_create[i:i + batch_size]
    #                     products = self.env["product.template"].sudo().create(batch)
    #                     created_products.extend(products)
    #                     created_count += len(products)
    #
    #                     if created_count % 500 == 0:
    #                         _logger.info(f"Created {created_count}/{len(to_create)} products...")
    #
    #                 _logger.info(f"Created {created_count} products")
    #
    #             # Step 6: Process images and variants SEQUENTIALLY (no threading with DB operations)
    #             _logger.info("Starting image and variant import...")
    #             processed = 0
    #
    ...
    #                     if processed % 100 == 0:
    #                         _logger.info(
    #                             f"Processed {processed}/{len(created_products)} products for images/variants...")
    #                         # Commit periodically to avoid long-running transactions
    #                         self.env.cr.commit()
    #
    #                 except Exception as e:
    #                     _logger.error(f"Error processing product {product.id}: {e}")
    #                     # Continue processing other products
    #                     continue
    #
    #             elapsed = time.time() - start_time
    #             _logger.info(
    #                 f"Product import completed in {elapsed:.2f}s. Created: {created_count}, Skipped: {skipped_count}")
    #
    #             # Log completion
    #             try:
    #                 with self.pool.cursor() as new_cr:
    #                     new_env = api.Environment(new_cr, self.env.uid, self.env.context)
    #                     new_env["cr.data.processing.log"].sudo()._log_data_processing(
    #                         cr_shop_id=store.id,
    #                         cr_message=f"Product import complete: {created_count} created, {skipped_count} skipped in {elapsed:.2f}s",
    #                         record_count=created_count,
    #                         status="success",
    #                         timespan=str(fields.Datetime.now()),
    #                         initiated_at=str(fields.Datetime.now()),
    ...
    #                 _logger.error(f"Failed to log success: {log_error}")
    #
    #             return {
    #                 "type": "ir.actions.client",
    #                 "tag": "display_notification",
    ...
    #                     "title": _("Import Successful"),
    #                     "message": _(
    #                         f"Products Imported: {created_count} created, {skipped_count} skipped in {elapsed:.2f}s"),
    #                     "sticky": False,
    #                 },
    #             }
    #
    ...
    #                     new_env["cr.data.processing.log"].sudo()._log_data_processing(
    #                         cr_shop_id=store.id,
    #                         cr_message="Failed to import products from BigCommerce.",
    #                         record_count=0,
    #                         status="failure",
    #                         timespan=str(fields.Datetime.now()),
    #                         initiated_at=str(fields.Datetime.now()),
    ...
    #                     new_cr.commit()
    #             except Exception as log_error:
    #                 _logger.error(f"Failed to log error: {log_error}")
    #
    #             raise UserError(error_msg)

    # def import_bigcommerce_products(self):
    #     """Ultra-optimized parallel import with aggressive batching and caching."""
    #     import time
    #     import concurrent.futures
    #     from collections import defaultdict
    #
    ...
    #             all_products = first_page_data
    #
    #             # Fetch remaining pages with increased workers
    #             if total_pages > 1:
    ...
    #             _logger.info(f"Fetched all {len(all_products)} products in {time.time() - start_time:.2f}s")
    #
    #             # Step 2: Pre-load ALL reference data with optimized queries
    #             shop_all_category_id = self.env["product.category"].sudo().search([
    #                 ("bigcommerce_store_id", "=", store.id),
    #                 ("name", "ilike", "shop all"),
    #             ], limit=1).id
    ...
    #                 brand_map = {b.brand_id: b.id for b in brands}
    #
    #             # Step 3: Prepare batch data (filter new products only)
    #             to_create = []
    #             skipped_count = len(existing_map)
    #
    #             for item in all_products:
    #                 if str(item["id"]) in existing_map:
    #                     continue
    ...
    #                 to_create.append(product_vals)
    #
    #             # Step 4: Create products in larger batches with explicit commits
    #             created_products = []
    #             created_count = 0
    #
    #             if to_create:
    #                 _logger.info(f"Creating {len(to_create)} products in optimized batches...")
    #
    #                 # Use larger batches (200 instead of 100)
    #                 batch_size = 200
    #
    #                 for i in range(0, len(to_create), batch_size):
    #                     batch = to_create[i:i + batch_size]
    #
    #                     # Create with context to skip unnecessary recomputations
    ...
    #                     created_count += len(products)
    #
    #                     # Commit every batch to avoid transaction bloat
    #                     self.env.cr.commit()
    #
    #                     if created_count % 500 == 0:
    #                         _logger.info(f"Created {created_count}/{len(to_create)} products...")
    ...
    #                 _logger.info(f"Created {created_count} products")
    #
    #             # Step 5: Process images and variants in optimized parallel batches
    #             if created_products:
    #                 _logger.info("Starting optimized parallel image and variant import...")
    #
    #                 def process_product_batch(product_batch):
    #                     """Process a batch of products with their own cursor."""
    #                     processed = []
    ...
    #                 ]
    #
    #                 # Process batches in parallel with 10 workers
    #                 with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
    #                     futures = [
    #                         executor.submit(process_product_batch, batch)
    ...
    #                             _logger.info(f"Processed {total_processed}/{len(product_ids)} products...")
    #
    #             elapsed = time.time() - start_time
    #             _logger.info(
    #                 f"Product import completed in {elapsed:.2f}s. Created: {created_count}, Skipped: {skipped_count}"
    #             )
    #
    #             # Log completion in separate transaction to avoid blocking
    #             try:
    #                 with self.env.registry.cursor() as new_cr:
    #                     new_env = api.Environment(new_cr, self.env.uid, self.env.context)
    #                     new_env["cr.data.processing.log"].sudo()._log_data_processing(
    ...
    #                     new_cr.commit()
    #             except Exception as log_error:
    #                 _logger.warning(f"Failed to create log entry: {log_error}")
    #
    #             return {
    #                 "type": "ir.actions.client",
    #                 "tag": "display_notification",
    ...
    #                     new_cr.commit()
    #             except Exception as log_error:
    #                 _logger.warning(f"Failed to create error log: {log_error}")
    #
    #             raise UserError(error_msg)
    #
    #     return None
    ...
    # def import_bigcommerce_products(self):
    #     """Ultra-fast parallel import of products with pagination."""
    #     import time
    #     import concurrent.futures
    ...
    #                 _logger.info(f"Created {created_count} products")
    #
    #                 _logger.info("Starting parallel image and variant import...")
    #
    #                 def process_product(product):
    #                     try:
    #                         product.with_context(prefetch_fields=False).import_product_image()
    #                         product.with_context(prefetch_fields=False).import_bigcommerce_variants()
    #                         return True
    #                     except Exception as e:
    #                         _logger.error(f"Error processing product {product.id}: {e}")
    #                         return False
    #
    #                 with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
    #                     futures = [executor.submit(process_product, p) for p in created_products]
    #                     processed = 0
    #                     for future in concurrent.futures.as_completed(futures):
    #                         future.result()
    #                         processed += 1
    ...
    #                     "message": f"Products Imported: {created_count} created, {skipped_count} skipped in {elapsed:.2f}s",
    #                     "sticky": False,
    #                 },
    #             }
    #
    ...
    #                 _logger.info(f"Created {created_count} products")
    #
    #             # Process images and variants sequentially (NO THREADING)
    #             if created_products:
    #                 _logger.info("Starting image and variant import...")
    #                 processed = 0
    #
    ...
    #                     try:
    #                         # Import images
    #                         product.sudo().import_product_image()
    #
    #                         # Import variants
    #                         product.sudo().import_bigcommerce_variants()
    #
    #                         processed += 1
    #
    ...
    #
    #                     except Exception as e:
    #                         _logger.error(f"Error processing product {product.id} ({product.name}): {e}")
    #                         continue
    #
    #                 # Final commit
    #                 self.env.cr.commit()
    #                 _logger.info(f"Completed processing {processed}/{len(created_products)} products")
    #
    #             elapsed = time.time() - start_time
    #             _logger.info(
    #                 f"Product import completed in {elapsed:.2f}s. Created: {created_count}, Skipped: {skipped_count}")
    ...
    # models/bigcommerce_store.py

    # def get_import_progress(self, import_type):
    #     """Get current import progress for display."""
    #     store_id = self.id
    ...
    #     )
    #
    #     total = int(self.env['ir.config_parameter'].sudo().get_param(
    #         f'{prefix}.total.{store_id}', '0'
    #     ))
    #
    #     created = int(self.env['ir.config_parameter'].sudo().get_param(
    #         f'{prefix}.created.{store_id}', '0'
    #     ))
    #
    #     updated = int(self.env['ir.config_parameter'].sudo().get_param(
    ...
    #     error_msg = ''
    #     if status == 'failed':
    #         error_msg = self.env['ir.config_parameter'].sudo().get_param(
    ...
    #
    #     total_expected = int(self.env['ir.config_parameter'].sudo().get_param(
    ...
    # In your bigcommerce.store model, add this method:
    ...
    def _calculate_elapsed_time(self, store_id, import_type, status, prefix):
        """Calculate elapsed time for import."""
        start_time_str = self.env['ir.config_parameter'].sudo().get_param(
            f'{prefix}.start_time.{store_id}', ''
        )
    ...
    def _process_bigcommerce_order(self, order_data, headers):
        ...
        print('len : ', len(shipping_addresses))
    ...
    def is_customer_exist(self, order_data):
        print('order_data["customer_id"] : ',order_data["customer_id"])
        customer_query = """
