...
class BigCommerceStore(models.Model):
    ...
    def action_export_products_to_bigcommerce(self):
        ...
        for payload in product_payloads:
            ...
            if response.status_code in [200, 201]:
                ...
                _logger.info(f"product_name : {product_name}")
                product_id = next(
                ...
                _logger.info(f"product.bigcommerce_product_id : {product.bigcommerce_product_id}")
                product.bigcommerce_store_id = self.id
                _logger.info(f"product.bigcommerce_store_id : {product.bigcommerce_store_id}")
                ...
                if product.attribute_line_ids:
                    _logger.info(f"yes...")
                    _logger.info(f"product.attribute_line_ids : {product.attribute_line_ids}")
                    ...
                    for variant in product.product_variant_ids:
                        ...
                        for attr_line in product.attribute_line_ids:
                            has_value = variant.product_template_attribute_value_ids.filtered(
                                lambda ptav: ptav.attribute_id == attr_line.attribute_id
                            )
                            if not has_value:
                                missing_attrs.append(attr_line.attribute_id.name)

                        if missing_attrs:
                            _logger.warning(
                                f"Skipping variant {variant.id}: missing attributes {missing_attrs}"
                            )
                            continue
                        ...
                        try:
                            ...
                            _logger.warning(f"Failed to export variant {variant.id}: {str(e)}")
                            fail += 1
                else:
        ...
        return {
            ...
            "params": {
                "title": _("Export Complete"),
                "message": _("Successfully exported %d products to BigCommerce. Failed: %d")
                           % (created_count, fail),
                "type": "success",
                "sticky": False,
            },
        ...
        if order_data.get("status") == "Shipped":
            ...
            if picking:
                ...
                if shipped_items and any(
                    move.quantity > 0 for move in picking.move_ids
                ):
                    try:
    ...
    def _get_applicable_tax(self, country_id, state_id, zip_code, tax_class='0'):
        """
        Get applicable tax for a given location and tax class.
        ODOO 19: Rewritten to work with new fiscal position structure (no account.fiscal.position.tax)
        """
        ...
        def _get_tax_from_fp(fp):
            """Extract applicable tax from fiscal position"""
            if not fp:
                return None

            # ODOO 19: Access tax_ids directly from fiscal position (many-to-many field)
            taxes = fp.tax_ids
            ...
            # Priority: Match by tax class first
            target_taxes = taxes.filtered(
                lambda t: str(t.bigcommerce_tax_class_id or '0') == str(tax_class)
            )
            if target_taxes:
                return target_taxes[0]

            # Fallback: Return any matching tax
            if taxes:
                return taxes[0]
        ...
        if fpos_from_postal:
            ...
            if tax:
                _logger.info(f"[BigCommerce] Tax matched via postal code: {tax.name}")
                return tax
        ...
        if fpos:
            _logger.info(f"[BigCommerce] Found fiscal position (country+state): {fpos.name}")
        ...
        if fpos:
            _logger.info(f"[BigCommerce] Found fiscal position (country only): {fpos.name}")
            tax = _get_tax_from_fp(fpos)
            if tax:
                return tax

        # Priority 4: fallback (global/no country)
        fpos_list = FiscalPosition.search([
            ("country_id", "=", False),
            ("bigcommerce_store_id", "=", self.id),
            ("company_id", "=", self.env.company.id),
        ])
        ...
        for fp in fpos_list:
            ...
            if tax:
                _logger.info(f"[BigCommerce] Tax matched via global fiscal position: {tax.name}")
                return tax

        _logger.warning(
            f"[BigCommerce] No applicable tax found for country={country_id}, state={state_id}, zip={zip_code}, tax_class={tax_class}")
        return None
                    ...
                    for item in shipped_items:
                        ...
                        if product:
                            ...
                            if move:
                                move.quantity = item["quantity"]
                                _logger.info(
                                    f"[BigCommerce] Set quantity {item['quantity']} for move: {move}"
                                )
                else:
                    _logger.info(
    ...
    def _sync_tax_zones_and_rates(self):
        ...
        for zone in zones:
            ...
            if is_global:
                ...
                # ODOO 19: Use tax_ids directly with (6, 0, tax_ids_list) instead of tax_src_id/tax_dest_id
                fp_vals = {
                    "name": fp_name,
                    "company_id": self.env.company.id,
