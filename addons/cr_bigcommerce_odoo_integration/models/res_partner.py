...
class ResPartner(models.Model):
    ...
    def action_update_customer_bigcommerce(self):
        ...
        if response.status_code in [200, 204]:
            ...
            raise UserError(
                _("Failed to update customer in BigCommerce: %s") % error_msg
            )
    ...
    def action_export_to_bigcommerce(self):
        ...
        if not store.access_token or not store.store_hash:
            raise UserError(
                _("Missing API credentials on the linked BigCommerce store.")
            )
                ...
                self.write(
                    {
                        ...
                        "bigcommerce_registration_ip": cust_data.get(
    ...
    def action_delete_customer_from_bigcommerce(self):
        ...
        params = {"id:in": str(self.bigcommerce_customer_id)}
    ...
    def action_delete_customers_from_bigcommerce(self):
        ...
        for i in range(batches):
            ...
            params = {"id:in": ",".join(customer_ids)}
        ...
        return {
            ...
            "params": {
                ...
                "message": _(
                    f"Deleted {total} customers from BigCommerce in {batches} batch(es)."
                ),
                "type": "success",
                "sticky": False,
            },
    ...
    def action_create_bigcommerce_customer_address(self):
        ...
        if not self.bigcommerce_customer_id:
            raise UserError(
                _("Customer does not have a linked BigCommerce customer ID.")
            )
    ...
    def action_update_bigcommerce_address(self):
        ...
        if not self.bigcommerce_customer_id:
            raise UserError(
                "This address is not linked to a customer with a BigCommerce ID."
            )

        first_name, last_name = payment_utils.split_partner_name(self.name)
        address_data = {
            "id": int(self.bigcommerce_customer_id),
            "first_name": first_name or self.parent_id.name.split()[0]
            if self.parent_id.name
            else "Unknown",
            "last_name": last_name or self.parent_id.name.split()[-1]
            if self.parent_id.name
            else "Customer",
    ...
    def action_delete_bigcommerce_address(self):
        ...
        params = {"id:in": self.bigcommerce_customer_id}
...
class ResUsers(models.Model):
    ...
    def _notification_channel(
        ...
        """Send a UI notification via ImBus in Odoo 18 (even from controller)."""
        _logger.info(
            "Preparing to send notification: type=%s, title=%s, message=%s, sticky=%s, target=%s",
            type,
            title,
            message,
            sticky,
        ...
        self.env["bus.bus"]._sendone(target, "simple_notification", bus_message)
