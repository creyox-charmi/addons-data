...
class DeliveryCarrier(models.Model):
    ...
    bc_method_id = fields.Integer(string="BC Method ID", readonly=True)
    ...
    exclude_fixed_shipping_products = fields.Boolean(
        string="Exclude Fixed Shipping Products"
    )
    ...
    def action_refresh_shipping_method(self):
        ...
        if response.status_code != 200:
            raise UserError(
                _("Failed to fetch shipping method.\nStatus: %s\nResponse: %s")
        ...
        self.write(
            {
                ...
                "exclude_fixed_shipping_products": carrier_options.get(
                    "exclude_fixed_shipping_products", "0"
                )
                == "1",
                "use_discounted_sub_total": carrier_options.get(
                    "use_discounted_sub_total", "0"
                )
                == "1",
        ...
        return {
            ...
            "params": {
                ...
                "message": _(
                    "Successfully refreshed shipping method data from BigCommerce."
                ),
                "type": "success",
                "sticky": False,
            },
    ...
    def action_export_shipping_method(self):
        ...
        if self.type == "perorder" or self.type == "peritem":
            ...
            settings = {
                "carrier_options": {
                    ...
                    "exclude_fixed_shipping_products": "1"
                    if self.exclude_fixed_shipping_products
                    else "0",
                    "use_discounted_sub_total": "1"
                    if self.use_discounted_sub_total
                    else "0",
                    "packaging": [],
                }
            }
        ...
        payload = {
            ...
            "handling_fees": {"fixed_surcharge": str(self.fixed_surcharge)},
        }
        ...
        if self.channel_ids_text:
            try:
                channel_ids = [
                    int(cid.strip())
                    for cid in self.channel_ids_text.split(",")
        ...
        if response.status_code != 200:
            raise UserError(
                _("Failed to export shipping method.\nStatus: %s\nResponse: %s")
                % (response.status_code, response.text)
            )
    ...
    def action_delete_shipping_method(self):
        ...
        if not self.store_id or not self.zone_id or not self.bc_method_id:
            raise UserError(
                _("Store, Zone, and Method ID must be set to delete shipping method.")
            )
        ...
        if response.status_code not in [200, 204]:
            raise UserError(
                _("Failed to delete shipping method.\nStatus: %s\nResponse: %s")
                % (response.status_code, response.text)
            )
        ...
        return {
            ...
            "params": {
                ...
                "message": _(
                    "Shipping method successfully deleted from BigCommerce and Odoo."
                ),
                "type": "success",
                "sticky": False,
            },
    ...
    def action_bulk_refresh_shipping_methods(self):
        for record in self:
            try:
                record.action_refresh_shipping_method()
            except Exception as e:
                _logger.exception(
                    "Shipping method refresh failed for %s: %s", record.name, str(e)
                )

    def action_bulk_export_shipping_methods(self):
        for record in self:
            try:
                record.action_export_shipping_method()
            except Exception as e:
                _logger.exception(
                    "Failed to export shipping method ID %s: %s", record.id, str(e)
                )
