...
class ProductTemplate(models.Model):
    ...
    def import_bigcommerce_variants(self):
        ...
        if response.status_code != 200:
            _logger.error(
                "Failed to fetch variants from BigCommerce for product %s: %s",
                product_id,
                response.text,
            )
        ...
        _logger.info(
            "Fetched %d variant(s) from BigCommerce for product ID: %s",
            len(bigcommerce_variants),
            product_id,
        )
        ...
        for variant in bigcommerce_variants:
            for option in variant.get("option_values", []):
                ...
                value = AttributeValue.search(
                    [("name", "=", attr_value), ("attribute_id", "=", attribute.id)],
                    limit=1,
                )
                if not value:
                    ...
                    _logger.info(
                        "Created new attribute value: %s (Attribute: %s)",
                        attr_value,
                        attr_name,
                    )
                ...
                if not attr_line:
                    ...
                    _logger.info(
                        "Created template attribute line for %s with value %s",
                        attr_name,
                        attr_value,
                    )
                elif value.id not in attr_line.value_ids.ids:
                    attr_line.value_ids = [
                        Command.link(val.id) for val in attr_line.value_ids + value
                    ]
                    _logger.info(
                        "Updated attribute line %s with new value %s",
                        attr_name,
                        attr_value,
                    )
        ...
        for variant in bigcommerce_variants:
            ...
            for rec in self.product_variant_ids:
                ...
                for value in rec.product_template_attribute_value_ids:
                    if variant_option_names.get(value.attribute_id.name) != value.name:
                        matched = False
                        break

                if matched:
                    rec.bigcommerce_store_id = self.bigcommerce_store_id.id
                    rec.bigcommerce_product_attribute_id = variant.get("id")
                    rec.bigcommerce_product_id = variant.get("product_id")
                    rec.bigcommerce_sku = variant.get("sku")
                    rec.bigcommerce_sku_id = variant.get("sku_id")
                    rec.default_code = variant.get("sku")
                    break
        ...
        for product in self:
            ...
            try:
                category_ids = (
                    ...
                    if product.bigcommerce_categories
                    else []
                )
            except Exception:
                raise UserError(
                    "Invalid category IDs. Ensure they are comma-separated integers."
                )
            ...
            payload = {
                ...
                "inventory_level": product.bigcommerce_inventory_level
                or int(product.qty_available),
    ...
    def bigcommerce_update_product(self):
        for product in self:
            if not product.bigcommerce_product_id:
                raise UserError(
                    _("Missing BigCommerce Product ID for product %s") % product.name
                )

            if (
                not product.bigcommerce_store_id
                or not product.bigcommerce_store_id.store_hash
                or not product.bigcommerce_store_id.access_token
            ):
                raise UserError(
                    _("Missing BigCommerce store configuration for product %s")
                    % product.name
                )
            ...
            payload = {
                ...
                "product_tax_code": product.bigcommerce_product_tax_code or "",
                "categories": [
                    int(c) for c in product.bigcommerce_categories.split(",")
                ]
                if product.bigcommerce_categories
                else [],
                "inventory_level": product.bigcommerce_inventory_level
            ...
            if response.status_code not in [200, 201]:
                raise UserError(
                    _("Failed to update product in BigCommerce: %s") % response.text
                )
    ...
    def action_delete_product_from_bigcommerce(self):
        ...
        if response.status_code == 204:
            ...
            raise UserError(
                f"Failed to delete product from BigCommerce: {response.status_code} - {response.text}"
            )
    ...
    def export_bigcommerce_product_images(self):
        ...
        if self.image_1920:
            ...
            files = {"image_file": ("main_image.jpg", img_binary, "image/jpeg")}
    ...
    def create_bigcommerce_product_option(self, product_id, attribute):
        """
        Create a product option in BigCommerce and save the option/value IDs.

        Args:
            product_id: product.template record
            attribute: product.attribute record
        """
        store = self.bigcommerce_store_id

        if not store or not product_id.bigcommerce_product_id:
            raise UserError("Product must be exported to BigCommerce first.")
        ...
        payload = {
            ...
            "type": "dropdown",  # Use 'dropdown', 'radio_buttons', 'swatch', etc.
            "option_values": [{"label": val.name} for val in attribute.value_ids],
        }

        _logger.info(
            f"Creating BigCommerce option '{attribute.name}' for product {product_id.bigcommerce_product_id}"
        )
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if not data:
                    raise UserError("No data returned from BigCommerce option creation")

                # Save the option ID to the attribute
                attribute.bigcommerce_option_id = data["id"]
                _logger.info(f"Saved BigCommerce option ID {data['id']} to attribute {attribute.name}")

                # Save value IDs to each attribute value
                for option_value in data.get("option_values", []):
                    # Find the matching attribute value by label
                    attr_value = attribute.value_ids.filtered(
                        lambda v: v.name == option_value["label"]
                    )

                    if attr_value:
                        # Save the BigCommerce value ID
                        attr_value.bigcommerce_value_id = option_value["id"]
                        _logger.info(
                            f"Saved BigCommerce value ID {option_value['id']} to "
                            f"attribute value {attr_value.name}"
                        )
                    else:
                        _logger.warning(
                            f"Could not find attribute value '{option_value['label']}' "
                            f"for attribute {attribute.name}"
                        )

                _logger.info(
                    f"Successfully created BigCommerce option {attribute.name} with ID {data['id']}"
                )
                return data["id"]

            elif response.status_code == 409:
                # Conflict - option might already exist
                error_data = response.json()
                error_msg = error_data.get("title", "Conflict error")
                _logger.warning(f"Option already exists: {error_msg}")
                raise UserError(
                    f"Option '{attribute.name}' may already exist in BigCommerce. "
                    f"Error: {error_msg}"
                )
            else:
                _logger.error(
                    f"Failed to create option {attribute.name}: {response.status_code} - {response.text}"
                )
                raise UserError(
            ...
            _logger.error(f"Request exception creating option: {str(e)}")
            raise UserError(f"Network error creating option: {str(e)}")
        except Exception as e:
            _logger.error(f"Unexpected error creating option: {str(e)}")
            raise UserError(f"Unexpected error creating option: {str(e)}")



    def export_bigcommerce_variant(self, product_id, variant):
        """
        Export a single product variant to BigCommerce with all required option values.

        Args:
            product_id: product.template record
            variant: product.product record (the variant)
        """
        store = self.bigcommerce_store_id
        ...
        # Iterate through each attribute line (option) in the product
        for attr_line in product_id.attribute_line_ids:
            attribute = attr_line.attribute_id
            option_id = attribute.bigcommerce_option_id

            if not option_id:
                _logger.error(
                    f"Attribute {attribute.name} does not have a BigCommerce option ID. "
                    f"Make sure the option was created in BigCommerce."
                )
            ...
            # Find the attribute value for this variant
            variant_attr_value = variant.product_template_attribute_value_ids.filtered(
                lambda ptav: ptav.attribute_id == attribute
            )

            if not variant_attr_value:
                _logger.warning(
                    f"Variant {variant.id} ({variant.name}) missing attribute value "
                    f"for {attribute.name}"
                )
                missing_attributes.append(f"{attribute.name} (no value on variant)")
                continue

            # Get the BigCommerce value ID
            value_id = variant_attr_value[0].product_attribute_value_id.bigcommerce_value_id

            if not value_id:
                _logger.error(
                    f"Attribute value {variant_attr_value[0].product_attribute_value_id.name} "
                    f"does not have a BigCommerce value ID. "
                    f"Make sure the value was created in BigCommerce."
                )
                missing_attributes.append(
                    f"{attribute.name}/{variant_attr_value[0].product_attribute_value_id.name}"
                )
                continue
            ...
            _logger.info(
                f"Variant {variant.id}: Adding option {option_id} ({attribute.name}) "
                f"with value {value_id} ({variant_attr_value[0].product_attribute_value_id.name})"
            )

        # Validate we have option values for all attributes
        if missing_attributes:
            error_msg = (
                f"Variant {variant.id} ({variant.name}) is missing values for: "
                f"{', '.join(missing_attributes)}\n\n"
                f"Product has {len(product_id.attribute_line_ids)} attributes:\n"
            )
            for attr_line in product_id.attribute_line_ids:
                error_msg += f"- {attr_line.attribute_id.name}\n"

            error_msg += (
                f"\nVariant has {len(option_values)} option values.\n"
                f"All {len(product_id.attribute_line_ids)} attributes must have values."
            )
        ...
        payload = {
            ...
            "price": variant.list_price or product_id.list_price or 0.0,
        }

        _logger.info(
            f"Exporting variant {variant.id} ({variant.name}) with payload: {payload}"
        )
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if not data:
                    raise UserError("No data returned from BigCommerce variant creation")

                # Update variant with BigCommerce IDs
                variant.write({
                    "bigcommerce_store_id": store.id,
                    "bigcommerce_product_attribute_id": data.get("id"),
                ...
                _logger.info(
                    f"Variant {variant.id} exported successfully with ID {data.get('id')}"
                )
                ...
                if variant.image_1920:
                    try:
                        ...
                        files = {"image_file": ("variant_image.jpg", img_binary, "image/jpeg")}

                        image_url = (
                            f"https://api.bigcommerce.com/stores/{store.store_hash}/"
                            f"v3/catalog/products/{product_id.bigcommerce_product_id}/"
                            f"variants/{data['id']}/image"
                        )
                        image_headers = {
                            "X-Auth-Token": store.access_token,
                            "Accept": "application/json",
                        ...
                        img_response = requests.post(image_url, headers=image_headers, files=files)

                        if img_response.status_code not in [200, 201]:
                            _logger.warning(
                                f"Failed to upload variant image for {variant.id}: "
                                f"{img_response.text}"
                            )
                        else:
                            _logger.info(f"Variant image uploaded successfully for {variant.id}")
                    except Exception as e:
                        _logger.warning(f"Error uploading variant image: {str(e)}")

                return data.get("id")

            elif response.status_code == 409:
                # Conflict error - likely due to missing option values
                error_data = response.json()
                error_msg = error_data.get("errors", {}).get("option_values", "Unknown error")
                _logger.error(f"Variant export conflict: {error_msg}")
                raise UserError(
                    f"Variant export failed with conflict: {error_msg}\n\n"
                    f"Make sure all attributes have option values assigned. "
                    f"Current option values sent: {option_values}"
                )
            else:
                _logger.error(f"Variant export failed: {response.status_code} - {response.text}")
                raise UserError(
                    f"Failed to export variant to BigCommerce:\n"
                    f"Status: {response.status_code}\n"
                    f"Error: {response.text}"
                )

        except requests.exceptions.RequestException as e:
            _logger.error(f"Request exception exporting variant: {str(e)}")
            raise UserError(f"Network error exporting variant: {str(e)}")
        except Exception as e:
            _logger.error(f"Unexpected error exporting variant: {str(e)}")
            raise UserError(f"Unexpected error exporting variant: {str(e)}")
