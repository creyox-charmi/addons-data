...
class ProductTemplate(models.Model):
    ...
    # def import_product_image(self):
    #     """Optimized image import with timeout and error handling."""
    #     store = self.bigcommerce_store_id
    #     product_id = self.bigcommerce_product_id
    #     url = f"https://api.bigcommerce.com/stores/{store.store_hash}/v3/catalog/products/{product_id}/images"
    #     headers = {
    ...
    #             return
    #
    #         # Only process first 5 images to save time
    #         for idx, img in enumerate(sorted(images[:5], key=lambda i: i.get("sort_order", 0))):
    #             img_url = img.get("url_standard") or img.get("url_zoom")
    #             if not img_url:
    #                 continue
    #
    ...
    #                 _logger.warning(f"Failed to download image for product {self.id}: {e}")
    #                 continue
    #
    #     except Exception as e:
    #         _logger.warning(f"Failed to fetch images for product {self.id}: {e}")

    # def import_bigcommerce_variants(self):
    #     """Optimized variant import with caching and bulk operations."""
    #     store = self.bigcommerce_store_id
    #     product_id = self.bigcommerce_product_id
    #     base_url = f"https://api.bigcommerce.com/stores/{store.store_hash}/v3/catalog/products/{product_id}/variants"
    #
    ...
    #
    #         bigcommerce_variants = response.json().get("data", [])
    #         if not bigcommerce_variants:
    #             return
    #
    #         # Pre-fetch all attributes and values
    #         Attribute = self.env["product.attribute"]
    #         AttributeValue = self.env["product.attribute.value"]
    #         TemplateAttributeLine = self.env["product.template.attribute.line"]
    #
    ...
    #         value_cache = {}
    #
    #         # Step 1: Process attributes in bulk
    #         for variant in bigcommerce_variants:
    #             for option in variant.get("option_values", []):
    #                 attr_name = option["option_display_name"]
    #                 attr_value = option["label"]
    ...
    #                             {"name": attr_name, "create_variant": "always"}
    #                         )
    #                     attr_cache[attr_name] = attribute
    #
    ...
    #                             {"name": attr_value, "attribute_id": attribute.id}
    #                         )
    #                     value_cache[value_key] = value
    #
    #         # Step 2: Create attribute lines
    #         for attr_name, attribute in attr_cache.items():
    #             attr_line = TemplateAttributeLine.search(
    #                 [
    #                     ("product_tmpl_id", "=", self.id),
    #                     ("attribute_id", "=", attribute.id),
    ...
    #
    #             relevant_values = [
    #                 v for k, v in value_cache.items() if k.startswith(f"{attr_name}||")
    #             ]
    #
    #             if not attr_line:
    #                 TemplateAttributeLine.create({
    #                     "product_tmpl_id": self.id,
    #                     "attribute_id": attribute.id,
    #                     "value_ids": [Command.set([v.id for v in relevant_values])],
    #                 })
    #             else:
    #                 current_ids = set(attr_line.value_ids.ids)
    #                 new_ids = set(v.id for v in relevant_values)
    #                 if new_ids != current_ids:
    #                     attr_line.value_ids = [Command.set(list(new_ids))]
    #
    #         # Step 3: Recompute variants
    #         self._create_variant_ids()
    #
    #         # Step 4: Bulk update variants
    #         for variant in bigcommerce_variants:
    #             variant_option_names = {
    #                 ov["option_display_name"]: ov["label"]
    #                 for ov in variant.get("option_values", [])
    ...
    #             for rec in self.product_variant_ids:
    #                 matched = all(
    #                     variant_option_names.get(value.attribute_id.name) == value.name
    #                     for value in rec.product_template_attribute_value_ids
    #                 )
    #
    #                 if matched:
    ...
    #                         "bigcommerce_product_attribute_id": variant.get("id"),
    #                         "bigcommerce_product_id": variant.get("product_id"),
    #                         "bigcommerce_sku": variant.get("sku"),
    #                         "bigcommerce_sku_id": variant.get("sku_id"),
    #                         "default_code": variant.get("sku"),
    #                     })
    #                     break
    #
    #     except Exception as e:
    #         _logger.warning(f"Failed to import variants for product {self.id}: {e}")
    ...
    def create_bigcommerce_product_option(self, product_id, attribute):
        ...
        if response.status_code == 200:
            ...
            # Save value IDs
            option_value_map = {
                v["label"]: v["id"] for v in data.get("option_values", [])
            }
    ...
    def export_bigcommerce_variant(self, product_id, variant):
        ...
        for ptav in variant.product_template_attribute_value_ids:
            ...
            if not option_id or not value_id:
                raise UserError(
                    f"Missing option_id or value_id for {ptav.display_name}"
                )
            option_values.append({"option_id": option_id, "id": value_id})
        ...
        if response.status_code == 200:
            ...
            if variant.image_1920:
                ...
                image_url = f"https://api.bigcommerce.com/stores/{store.store_hash}/v3/catalog/products/{product_id.bigcommerce_product_id}/variants/{data['id']}/image"
                image_headers = {
                    "X-Auth-Token": store.access_token,
                    "Accept": "application/json",
                }

                response = requests.post(image_url, headers=image_headers, files=files)
