...
class Signature(models.Model):
    ...
    cr_hr_employee_document_id = fields.Many2one('hr.employee.document',  string='HR Employee Document')
    invisible_document = fields.Boolean(compute='_compute_visibility', store=True)


    @api.depends('cr_contact', 'cr_sale_order_name', 'cr_purchase_order_name', 'cr_invoice_name','cr_hr_employee_document_id')
    def _compute_visibility(self):
        """This method ensures that only one field is visible at a time."""

        for record in self:
            record.invisible_contact = bool(
                record.cr_sale_order_name or record.cr_purchase_order_name or record.cr_invoice_name or record.cr_hr_employee_document_id)
            record.invisible_sale_order = bool(
                record.cr_contact or record.cr_purchase_order_name or record.cr_invoice_name or record.cr_hr_employee_document_id)
            record.invisible_purchase_order = bool(
                record.cr_contact or record.cr_sale_order_name or record.cr_invoice_name or record.cr_hr_employee_document_id)
            record.invisible_invoice = bool(
                record.cr_contact or record.cr_sale_order_name or record.cr_purchase_order_name or record.cr_hr_employee_document_id)
            record.invisible_document = bool(
                record.cr_contact or record.cr_sale_order_name or record.cr_purchase_order_name or record.cr_invoice_name)
    ...
    def action_update_status(self):
        ...
        try:
            ...
            if self.cr_docs_policy == 'hierarchy':
                ...
                next_recipient = self.cr_recipients[next_index] if next_index < len(self.cr_recipients) else None
    ...
    def attach_sim_doc(self, current_recipient, recipient):
        ...
        for document in envelope_documents.envelope_documents:
            ...
            final_file_path = os.path.join(downloads_folder, f"{current_recipient.cr_name}_signed.pdf")
            shutil.move(temp_file_path, final_file_path)

            with open(final_file_path, 'rb') as pdf_file:
                document_data = pdf_file.read()
    ...
    def _attach_signed_documents_to_next_recipient(self):
        ...
        for document in envelope_documents.envelope_documents:
            ...
            final_file_path = os.path.join(
                downloads_folder, f"{recipients[last_signed_index].cr_name}_signed.pdf"
            )
            shutil.move(temp_file_path, final_file_path)
