...
class ResUser(models.Model):
    ...
    def open_docusign_auth(self):
        ...
        # Auto-fix redirect URL
        redirect_url = self.cr_redirect_url or ""
        if redirect_url.endswith("/"):
            redirect_url = redirect_url[:-1]

        # Append callback path only if not already present
        if not redirect_url.endswith("/docusign/auth/callback"):
            redirect_url = redirect_url + "/docusign/auth/callback"

        # Save fixed redirect URL back to field
        self.cr_redirect_url = redirect_url
