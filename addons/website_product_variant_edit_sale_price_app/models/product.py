...
class ProductTemplate(models.Model):
    ...
    def _get_combination_info(
        ...
        if self.env.context.get("website_id"):
            ...
            fpos = (
                self.env["account.fiscal.position"].sudo()._get_fiscal_position(partner)
            )
            ...
            if product and product._name == "product.product":
                if pricelist:
                    price = pricelist.with_context(
                        new_list_price=True
                    )._get_product_price(product, quantity_1)
                else:
                    price = list_price
                list_price = product.price_compute("list_price")[product.id]
            else:
                # For product.template, use the list_price field directly
...
class ProductProduct(models.Model):
    ...
    def _compute_product_lst_price(self):
        ...
        for product in self:
            if product.set_attribute_price == True:
                if to_uom:
                    new_list_price = product.uom_id._compute_price(
                        product.new_list_price, to_uom
                    )
                else:
    ...
    def _price_compute(
        ...
        res = super()._price_compute(
            price_type, uom=None, currency=None, company=None, date=False
        )
        prices = dict.fromkeys(self.ids, 0.0)
        for product in self:
            if price_type == "list_price":
...
class Pricelist(models.Model):
    ...
    def _compute_price_rule(
        self, products, quantity, uom=None, date=False, compute_price=True, **kwargs
    ):
        ...
        for product in products:
            ...
            price = suitable_rule._compute_price(
                product, quantity, target_uom, date=date, currency=self.currency_id
            )
