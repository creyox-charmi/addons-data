...
class Stock(models.Model):
    ...
    def _return_subcontracting_picking(self,po,description):
        receipt_line_dict = []
    ...
    def _action_done(self):
        ...
        for rec in self:
            if rec.picking_type_id and rec.picking_type_id.code == "outgoing":
                rec._return_subcontracting_picking(rec.origin,rec.move_ids_without_package.description_picking)
    ...
    def button_validate(self):
        ...
        if code == 'outgoing':
            x = self.env['stock.quant'].search([('lot_id','=',self.mrp_id.lot_producing_id.id)])
            if not x:
                for move in self.move_ids_without_package:
                    if move.product_id:
                        q = self.env['stock.quant'].create(
                            {
                                'product_id': move.product_id.id,
                                'available_quantity': 0,
                                'lot_id': self.mrp_id.lot_producing_id.id,
