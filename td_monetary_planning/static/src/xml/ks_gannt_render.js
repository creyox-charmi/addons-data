...
patch(ksGanttRenderer.prototype, {
    ...
    setup() {
        ...
        this._ksToggleTimePercentage = this._ksToggleTimePercentage.bind(this);
    },
    ...
    if (ks_data.length > 0) {
        ...
        if (lastTask.id === each_project_task.id) {
            ...
            lastTask.percentage = each_project_task.percentage;
        }
    }
},
    ...
    ks_renderGantt() {
        ...
        gantt.config.ks_show_target = false;
        gantt.config.ks_show_sale = false;
        gantt.config.ks_show_time_percentage = false;
        super.ks_renderGantt();
    },

    _ksToggleTimePercentage(ev) {
    if (!gantt.config.ks_show_time_percentage) {
        gantt.config.ks_show_time_percentage = true;
        this.ks_enable_button(ev);
    } else {
        gantt.config.ks_show_time_percentage = false;
        this.ks_disable_button(ev);
...
ks_task_dynamic_content() {
    ...
    gantt.templates.task_text = function taskTextTemplate(start, end, task) {
        ...
        if (gantt.config.ks_show_time_percentage && ks_task_text && task.type != 'project') {
            ks_task_text += " (" + task.percentage + "%)";
        }
    ...
    ks_data_update(each_task, ks_links, ks_data, ks_gantt_fields, parent_group_id) {
        ...
        var a = each_task[ks_gantt_fields.subtask_percentage];
        ...
        if (ks_data.length > 0) {
            ...
            if (lastTask.id === each_task[ks_gantt_fields.ks_task_id]) {
                ...
                lastTask.percentage = each_task[ks_gantt_fields.subtask_percentage];
            }
        }
    }
});
