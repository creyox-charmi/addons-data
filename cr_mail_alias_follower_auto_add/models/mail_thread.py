...
class MailThreadAutoFollower(models.AbstractModel):
    ...
    def _add_to_recipients_as_followers(self, msg_dict):
        """
        Extract email addresses from 'To' header and add them as followers
        """
        to_header = msg_dict.get('to', '')
        if not to_header:
            return

        # Extract email addresses from "To"
        email_pattern = r'[\w\.-]+@[\w\.-]+\.\w+'
        to_emails = re.findall(email_pattern, to_header)
                ...
                try:
                    ...
                    if record.exists():
                        ...
                        if all_recipients_header:
                            email_pattern = r'[\w\.-]+@[\w\.-]+\.\w+'
                            recipient_emails = re.findall(email_pattern, all_recipients_header)
                            ...
                            for email in recipient_emails:
                                ...
                                if not partner:
                                    name, _ = parseaddr(all_recipients_header)
                                    partner = partner_model.create({
                                        'name': name or email.split('@')[0],
                                        'email': email,
                                    })
