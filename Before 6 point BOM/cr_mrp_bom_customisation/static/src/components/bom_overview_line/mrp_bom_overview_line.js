...
patch(BomOverviewLine.prototype, {
    ...
    setup() {
        ...
        console.log('this.canEditApproval2 : ',this.canEditApproval2)
    },
...
async goToAction(id, model) {
        if (model === "product.product" || model === "product.template") {
            const mainBomIds = await this.ormService.call(
                ...
                "search",
                [[
                    ["product_id", "=", model === "product.product" ? id : false],
            ...
            if (mainBomIds && mainBomIds.length > 0) {
                const allChildren = await this.ormService.call(
                    "mrp.bom.line",
                    "search_read",
                    [[["bom_id", "=", mainBomIds[0]]]],
                    { fields: ["product_id"] }
                ...
                if (allChildren && allChildren.length > 0) {
                    for (const child of allChildren) {
                        const childProductId = child.product_id[0];

                        const childProduct = await this.ormService.call(
                            "product.product",
                            "read",
                            [childProductId],
                        ...
                        const childTemplateId = childProduct[0].product_tmpl_id[0];

                        const childBomIds = await this.ormService.call(
                            "mrp.bom",
                            "search",
                            [['|',
                                ...
                                '&',
                                ["product_id", "=", false],
                                ["product_tmpl_id", "=", childTemplateId]
                            ]],
                            { limit: 1 }
                        );

                        if (childBomIds && childBomIds.length > 0) {
                            return this.actionService.doAction({
                                type: "ir.actions.act_window",
                                res_model: "mrp.bom",
                                res_id: childBomIds[0],
                                views: [[false, "form"]],
                                target: "current",
                                context: {
                                    active_id: childBomIds[0],
                                },
                            });
                        }
                    }
