...
class SaleOrder(models.Model):
    ...
    @api.depends('state','cr_account_payment_line','cr_account_payment_line.state','cr_account_payment_line.amount','invoice_ids.payment_state','cr_invoice_payments_widget')
    def _compute_cr_payment(self):
        for order in self:
            cr_payment_state = 'draft'
            currency = order.currency_id

            total_paid = sum([payment.currency_id._convert(payment.amount,currency,order.currency_id) for payment in order.cr_account_payment_line.filtered(lambda pay: pay.state=='in_process' and pay.payment_type=='inbound' and pay.partner_id.id == order.partner_id.id)])
            if total_paid:
            ...
            if order.cr_account_payment_line:
                ...
                if order.state in ('sale','done'):
                    ...
                    for payment in order.cr_account_payment_line:
                        for line in payment.move_id.line_ids.filtered(lambda line: line.account_id.account_type in ('asset_receivable', 'liability_payable')):
