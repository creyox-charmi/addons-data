        ...
        for order in sale_orders:
            if order.cr_payment_state == 'paid':
                for invoice in (order.invoice_ids & invoices):
                    invoice.action_post()
                    lines = order.cr_account_payment_line.move_id.line_ids.filtered(lambda line: line.account_id.account_type in ('asset_receivable', 'liability_payable') and not line.reconciled)
