...
class TdRegisterPaymentWizard(models.TransientModel):
    ...
    partner_id = fields.Many2one('res.partner', string="Customer/Vendor", store=True, copy=False, ondelete='restrict',
                                 related='sale_order_id.partner_id')
    company_id = fields.Many2one('res.company', store=True, copy=False, related='sale_order_id.company_id')
    currency_id = fields.Many2one(comodel_name='res.currency', string='Currency', related='sale_order_id.currency_id',
                                  store=True, readonly=False, help="The payment's currency.")
    ...
    journal_id = fields.Many2one(comodel_name='account.journal', compute='_compute_journal_id', store=True,
                                 readonly=False, precompute=True, check_company=True,
    ...
    partner_bank_id = fields.Many2one(comodel_name='res.partner.bank', string="Recipient Bank Account", readonly=False,
                                      store=True, compute='_compute_partner_bank_id',
