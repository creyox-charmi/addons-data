...
class WebsiteForm(http.Controller):
    ...
    def submit_quote_form(self, **kwargs):
        ...
        pricelist = request.env['product.pricelist'].search(
            [
                ('currency_id', '=', currency_of_country.id)
            ]
            , limit=1
        )
        ...
        for i in range(len(product1)):
            product = request.env['product.product'].sudo().search([('id', '=', product1[i])], limit=1)

            if not product:
                return {'status': 'error', 'message': f"Product '{product1[i]}' not found."}
            ...
            sale_order_note = request.env['sale.order.line'].sudo().create({
    ...
    def send_mail(self, supervisor_emails, sale_order):
        ...
        """
        company = request.env["res.company"].browse(1)  # Assuming the first company, adjust as needed
        ...
        <body>
            <div class="email-container">
                <div class="email-header">
                    <h2>Your Quotation from {company.name}</h2>
                </div>
                ...
                <div class="attachment-info">
                    <p>You can download your quotation document in PDF format from Attachments page.</p>
                </div>
    ...
    """
        ...
        try:
            ...
            pdf_content, _ = report_action._render_qweb_pdf(report_name, res_ids=[sale_order.id])
