...
class CrmStageChangeHistory(models.Model):
    ...
    day_diff = fields.Integer(
        string="Day Diff", compute="_compute_day_diff", store=True
    )
    time_diff = fields.Float(
        string="Time Diff", compute="_compute_time_diff", store=True
    )
    total_time_diff = fields.Float(
        string="Total Time Diff", compute="_compute_total_time_diff", store=True
    )
