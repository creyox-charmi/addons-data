...
class StockLocation(models.Model):
    ...
    def action_update_location_to_bigcommerce(self):
        for rec in self:
            ...
            if response.status_code == 200:
                ...
                raise UserError(
                    _("Failed to update location. Response: %s") % response.text
                )
        return None
    ...
    def action_delete_location_from_bigcommerce(self):
        for rec in self:
            ...
            if rec.bc_location_id == 1:
                raise UserError(
                    _(
                        "Location ID 1 is the default shipping origin and cannot be deleted."
                    )
                )

            api_url = f"https://api.bigcommerce.com/stores/{store.store_hash}/v3/inventory/locations"
            params = {"location_id:in": int(self.bc_location_id)}
            ...
            if response.status_code == 200:
                ...
                return {
                    ...
                    "params": {
                        ...
                        "message": _(
                            "The location was successfully deleted from BigCommerce."
                        ),
                        "type": "success",
                        "sticky": False,
                    },
                ...
                raise UserError(
                    _("Failed to delete location. Response: %s") % response.text
                )
        return None
    ...
    def action_sync_inventory(self):
        ...
        return {
            ...
            "views": [(False, "form")],
            "context": {
                "default_location_ids": self.env.context.get("active_ids", self.ids)
            },
        }
        self.ensure_one()
        ...
        try:
            for item in items:
                ...
                for location in item["locations"]:
                    ...
                    quantity = (
                        location["available_to_sell"]
                        - location["settings"]["safety_stock"]
                    )
                    if quantity < 0:
                        quantity = 0
    ...
    def action_export_to_bigcommerce(self, store):
        ...
        for location in self:
            payload = [
                {
                    "code": location.name.replace(
                        " ", "_"
                    ).upper(),  # Unique identifier
            ...
            try:
                ...
                location.write(
                    {"transaction_id": transaction_id, "bigcommerce_store_id": store.id}
                )

                _logger.info(
                    "Location %s exported to BigCommerce successfully.", location.name
                )
            except requests.exceptions.HTTPError as errh:
                _logger.error(
                    "HTTPError exporting location %s: %s and %s",
                    location.name,
                    str(errh),
                    response.json(),
                )
                ...
                _logger.error(
                    "Unexpected error exporting location %s: %s and %s",
                    location.name,
                    str(e),
                    response.json(),
                )
