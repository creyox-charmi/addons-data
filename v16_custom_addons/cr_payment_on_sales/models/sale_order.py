...
class SaleOrder(models.Model):
    ...
    def _compute_cr_payment(self):
        for order in self:
            ...
            total_paid = sum([payment.currency_id._convert(payment.amount,currency,order.currency_id,payment.date) for payment in order.cr_account_payment_line.filtered(lambda pay: pay.state=='posted' and pay.payment_type=='inbound' and pay.partner_id.id == order.partner_id.id)])
            if total_paid:
