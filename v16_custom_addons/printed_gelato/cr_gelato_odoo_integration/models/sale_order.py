...
class SaleOrder(models.Model):
    ...
    def _action_cancel(self):
        ...
        _logger.info(f"==========================================")
        _logger.info(f"in if  action cancel from salae order")
        _logger.info(f"==========================================")
        try:
    ...
    def _is_mixing_of_gelato_and_non_gelato_products(self):
        ...
        # x = len(self.order_line.product_id)
        # _logger.info(f"==========================================")
        # _logger.info(f"length  >>>>>> {x} ")
        # _logger.info(f"==========================================")
        for data in self:
            for product in data.order_line.product_id:
                _logger.info(f"product  >>>>>> {product.name} ")
                _logger.info(f"default_codes  >>>>>> {product.default_code} ")
                if product.cr_product_uid:
                    _logger.info(f"==========================================")
                    _logger.info(f"in if  >>>>>> {product.cr_product_uid} ")
                    _logger.info(f"==========================================")
                    gelato_product += 1
                ...
                #    gelato_product += 1
                    ...
                    _logger.info(f"==========================================")
                    _logger.info(f"in else >>>>>> {product.name} ")
                    _logger.info(f"==========================================")
                    non_gelato_product += 1

        if gelato_product > 0 and non_gelato_product > 0:
            _logger.info(f"==========================================")
            _logger.info(f"in final if condition")
            _logger.info(f"==========================================")
        ...
        for data in self:
            ...
            for record in data.order_line:
                ...
                if record.product_id.cr_product_uid:
                    _logger.info(f">> {record.product_id.cr_product_uid}")
    ...
    def _generate_order_in_gelato(self):
        ...
        # if any(line.product_id.default_code in special_delivery_codes for line in self.order_line):
        #     shipment_method = 'normal'
        ...
        _logger.info(f"==================ORDER========================")
        _logger.info(f"ORDER : {payload}")
        _logger.info(f"==========================================")
        try:
            api_key = self.company_id.sudo().api_key_for_gelato
            ...
            if data:
                ...
                _logger.info(f"==========================================")
                _logger.info(f"self.gelato_order_id : {self.gelato_order_id}")
                _logger.info(f"==========================================")
        except UserError as e:
            raise UserError(_(
    ...
    def parse_datetime_with_timezone(self, date_string):
        # If the string has a timezone like '2025-04-22T07:45:16+00:00', we can parse it correctly
        try:
            return fields.Datetime.from_string(date_string)
        except ValueError:
            # Parse with timezone awareness using Python's datetime library
