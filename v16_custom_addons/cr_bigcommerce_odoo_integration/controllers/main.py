...
class BigCommerceWebhookController(http.Controller):
        ...
        try:
            if is_update:
                sale_order = (
                    ...
                    .sudo()
                    .search([("bigcommerce_order_id", "=", order_id)], limit=1)
                )
                if not sale_order:
                    _logger.info(
    ...
    def _process_product_created(self, store, data):
        ...
        try:
            ...
            _logger.info(
                "Checking if product %s already exists in Odoo (store %s)...",
                product_id,
                store.store_hash,
            )
            existing_product = (
            ...
            if existing_product:
                _logger.info(
                    "Product %s already exists: %s", product_id, existing_product.name
                )
                store.env["cr.data.processing.log"]._log_data_processing(
                    cr_shop_id=store.id,
                    record_count=1,
            ...
            _logger.info(
                "Fetching product details for product ID %s from BigCommerce...",
                product_id,
            )
            ...
            _logger.info(
                "Successfully fetched product details for product %s", product_id
            )
            ...
            _logger.info(
                "Created new product template: %s (BigCommerce product %s)",
                product.name,
                product_id,
            )
    ...
    def _process_product_updated(self, store, data):
        ...
        try:
            _logger.info(
                "Searching for product template with BigCommerce ID %s in store %s",
                product_id,
                store.store_hash,
            )
            product = (
    ...
    def _map_product_data(self, store, product_data):
        ...
        try:
            ...
            brand_value = (
                self._get_or_create_brand(store, brand_id) if brand_id else False
            )
            ...
            tax_class_id = str(product_data.get("tax_class_id"))
            tax = (
            ...
            if tax:
                product_vals["taxes_id"] = [(6, 0, [tax.id])]
    ...
    def _process_sku_inventory_updated(self, store, data):
        ...
        if not (product_id and variant_id):
            error_msg = (
                f"Missing product ID or variant ID in webhook data: {json.dumps(data)}"
            )
            _logger.error(error_msg)
        ...
        try:
            ...
            _logger.info(
                f"Fetched {len(locations)} inventory locations for store {store.store_hash}"
            )

            if not locations:
                _logger.warning(
                    f"No inventory locations found for store {store.store_hash}"
                )
            ...
            _logger.info(
                f"Searching for Odoo product variant: product_id={product_id}, variant_id={variant_id}"
            )
            product = (
                store.env["product.product"]
                .sudo()
            ...
            for location in locations:
                ...
                if not location["enabled"]:
                    _logger.info(
                        f"Skipping disabled location {location_id} in store {store.store_hash}"
                    )
                    continue
                ...
                for item in items:
                    ...
                    _logger.info(
                        f"Looking for stock location in Odoo for location_id={location_id}"
                    )
                    stock_location = (
                    ...
                    if not stock_location:
                        _logger.warning(
                            f"No matching stock location in Odoo for BC location_id={location_id}"
                        )
                        continue
            ...
            if not found:
                _logger.warning(
                    f"No matching inventory found for product ID {product_id}, variant ID {variant_id}"
                )
                # [logging block remains unchanged...]
