...
class PurchaseOrder(models.Model):
    ...
    invoice_payments_widget = fields.Binary(string='Payment Details', exportable=False,
                                            compute='_compute_invoice_payment_widget')
    total_amount = fields.Monetary(string='total_amount',compute='_compute_total_amount',store=True)

    @api.depends('amount_total')
    def _compute_total_amount(self):
        """Compute the total amount and amount due for the purchase order"""
        for record in self:
            record.total_amount = record.amount_total
            record.amount_due = record.amount_total

    def action_create_invoice(self):
        """Override action to set payment state to 'not_paid' when invoice is created"""
    ...
    def action_register_payment(self):
        """Action to register payment on the invoice lines of the purchase order"""
        return {
    ...
    @api.depends('invoice_ids.payment_state', 'invoice_ids.invoice_payments_widget')
    def _compute_invoice_payment_widget(self):
        """Compute the invoice payments widget data based on the state of the invoices"""
        is_overdue = 0
        today_date = datetime.now().date()

        #Loop through all purchase orders to check invoice states , If any overdue invoices are found, set the payment state of the order to 'overdue'
        for order in self:
