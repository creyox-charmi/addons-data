...
class PurchaseOrder(models.Model):
    ...
    cr_purchase_order_terms_and_condition_id = fields.Many2one(comodel_name='cr.terms.and.condition',
        ...
        if self.partner_id:
            line = self.env['cr.terms.and.condition'].search(
                [
                    ...
                    ('cr_model_reference', '=', 'purchase')
                ], limit=1)
            if line:
                self.cr_purchase_order_terms_and_condition_id = line.id
            else:
    ...
    def default_t_and_c(self):
        ...
        """
        is_terms_and_condition = self.env.user.company_id.cr_is_boolean_for_purchase_order
        cr_default_terms_and_condition = self.env.user.company_id.cr_purchase_order_terms_and_conditions_id
        if is_terms_and_condition:
            if cr_default_terms_and_condition:
                self.cr_purchase_order_terms_and_condition_id = cr_default_terms_and_condition.id
            else:
                self.cr_purchase_order_terms_and_condition_id = None
        else:
            self.cr_purchase_order_terms_and_condition_id = None

    @api.onchange('partner_id', 'cr_purchase_order_terms_and_condition_id')
    def add_note(self):
        """
        ...
        """
        if self.partner_id:
            source_language = self.cr_purchase_order_terms_and_condition_id.language
            destination_language = self.partner_id.lang
            notes = self.cr_purchase_order_terms_and_condition_id.terms_and_condition

            # No translation needed
            if source_language == destination_language:
                self.notes = notes
            else:
                if self.env.user.company_id.cr_is_auto_translate_for_purchase_order:
