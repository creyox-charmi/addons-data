...
class SaleOrder(models.Model):
    ...
    cr_sale_order_terms_and_condition = fields.Many2one(comodel_name='cr.terms.and.condition',
        ...
        if self.partner_id:
            line = self.env['cr.terms.and.condition'].search(
                [
                    ...
                    ('cr_model_reference', '=', 'sales')
                ], limit=1)
            if line:
                self.cr_sale_order_terms_and_condition = line.id
            else:
    ...
    def default_t_and_c(self):
        ...
        """
        is_terms_and_condition = self.env.user.company_id.cr_is_boolean_for_sale_order
        cr_default_terms_and_condition = self.env.user.company_id.cr_sale_order_terms_and_condition_id
        if is_terms_and_condition:
            if cr_default_terms_and_condition:
                self.cr_sale_order_terms_and_condition = cr_default_terms_and_condition.id
            else:
                self.cr_sale_order_terms_and_condition = None
        else:
            self.cr_sale_order_terms_and_condition = None

    @api.onchange('partner_id', 'cr_sale_order_terms_and_condition')
    def add_note(self):
        """
        ...
        """
        if self.partner_id:
            source_language = self.cr_sale_order_terms_and_condition.language
            destination_language = self.partner_id.lang
            notes = self.cr_sale_order_terms_and_condition.terms_and_condition

            # No translation needed
            if source_language == destination_language:
                self.note = notes
            else:
                if self.env.user.company_id.cr_is_auto_translate_for_sale_order:
