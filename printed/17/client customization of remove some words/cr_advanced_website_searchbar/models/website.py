...
class CrWebsite(models.Model):
    ...
    def _search_exact(self, search_details, search, limit, order):
        # if search:
        #     if isinstance(search, (set, list, tuple)):
        #         search = ' '.join(str(term) for term in search if isinstance(term, str))
        #         print(f"Converted search to string: {search}")
        ...
        # Filter out duplicate models, keeping the first occurrence
        # seen_models = set()
        # filtered_search_details = []
        # for search_detail in search_details:
        #     model = search_detail.get('model')
        #     if model not in seen_models:
        #         seen_models.add(model)
        #         filtered_search_details.append(search_detail)
        #     else:
        #         print(f"Skipping duplicate model: {model}")
        #
        # print('+++++++++filtered_search_details : ', filtered_search_details)
        #
        # for search_detail in filtered_search_details:
        #     print('+++++++++search_detail : ', search_detail)
        #     # No processing of results_data or blocked words; just keep the data as is
        #     results_data = search_detail.get('results_data', [])
        #     results = search_detail.get('results', self.env[search_detail.get('model')])
    ...
    def _search_render_results(self, search_details, limit):
        ...
        print("_search_render_results override with filtering logic")

        # Get blocked words from the current user's company
        company = self.env.company
        blocked_words = set(word.name.lower() for word in company.add_words_ids if word.name)
        ...
        for search_detail in search_details:
            ...
            #     search_detail['results'] = self.env['product.template'].browse([r.id for r in filtered_results])
