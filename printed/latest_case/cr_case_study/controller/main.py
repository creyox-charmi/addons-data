...
class CaseStudyController(http.Controller):
    ...
    def get_case_studies(self, **kwargs):
        ...
        for case in case_studies:
            case_study_data.append({
                ...
                'client_challenges_img': encode_image(case.client_challenges_img),
                'our_solution': case.our_solution,
                'our_solution_img': encode_image(case.our_solution_img),
                'impact': case.impact,
                'impact_img': encode_image(case.impact_img),
    ...
    # @http.route('/case_studies', type='http', auth='public', website=True)
    # def list_case_studies(self, **kwargs):
    #     case_studies = request.env['case.study'].search([])
    #     case_study_data = []
    #
    #     def encode_image(image):
    #         if not image:
    ...
    #         })
    #
    #     return request.render('your_module.case_study_page', {'case_studies': case_study_data})
    ...
    def case_study_detail(self, case_id, **kwargs):
        ...
        return request.render('cr_case_study.case_study_detail', case_study_data)

    @http.route('/case_study/review', auth="public", type="http", methods=['GET'])
    def random_blog_posts(self, **kwargs):
        """
        Method to retrieve random 4 blog posts from the 'blog.post' model
        """
        case_study_id = kwargs.get('id')
        print("case_study_id : ", case_study_id)
        # url_path = request.httprequest.path
        # print("url_path : ", url_path)
        case_study = request.env['case.study'].search([('id', '=', case_study_id)])
        print("case_study : ", case_study)
        case_study_data = []
        ...
        if case_study:
            ...
            return {'error': 'No case study found'}
