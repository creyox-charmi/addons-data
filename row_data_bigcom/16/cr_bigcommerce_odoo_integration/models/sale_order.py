...
class SaleOrder(models.Model):
    ...
    def _set_delivery_cost(self, carrier, amount):
        ...
        if self.state in ['draft', 'sent']:
            ...
            _logger.info("Created new delivery line for draft order '%s' with cost %.2f", self.name, amount)
        else:
        ...
        _logger.info("Set delivery cost %.2f on order '%s' for carrier '%s'", amount, self.name, carrier.name)
        return True
