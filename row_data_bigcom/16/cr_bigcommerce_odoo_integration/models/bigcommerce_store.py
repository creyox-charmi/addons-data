...
class BigCommerceStore(models.Model):
    ...
    def action_sync_store_info(self):
        ...
        for record in self:
            ...
            if not all(
                    [record.access_token, record.api_url, record.store_hash]):
                raise UserError(_("Please fill in Access Token, API URL, and Store Hash."))
    ...
    #             else:
    #                 print('in else')
    #                 url = f"https://api.bigcommerce.com/stores/{self.store_hash}/v3/catalog/products/{product.bigcommerce_product_id}/variants"
    #                 headers = {
    #                     "X-Auth-Token": self.access_token,
    #                     "Accept": "application/json",
    #                     "Content-Type": "application/json"
    #                 }
    #                 response_variant = requests.get(url, headers=headers, json=payload)
    #                 print('response_variant :  ',response_variant.text)
    #                 if response_variant.status_code == 200:
    #                     data_list = response_variant.json().get('data', [])
    #                     if data_list:
    #                         variant_data = data_list[0]
    #                         prod_var = self.env['product.product'].search([('product_tmpl_id', '=', product.id)],
    #                                                                       limit=1)
    #                         print('prod_var : ',prod_var)
    #                         if prod_var:
    #                             prod_var.write({
    #                                 'bigcommerce_product_attribute_id': variant_data.get('id'),  # Variant ID
    #                                 'bigcommerce_product_id': variant_data.get('product_id'),
    #                                 'bigcommerce_sku': variant_data.get('sku'),
    #                                 'bigcommerce_sku_id': variant_data.get('sku_id') or 0,
    #                                 'bigcommerce_store_id': self.id,
    #                             })
    #
    #
    ...
    def action_export_products_to_bigcommerce(self):
        ...
        for product in products:
            ...
            payload = {
                ...
                "categories": category_ids,
                "brand_id": product.bigcommerce_brand_id.brand_id if product.bigcommerce_brand_id else None,
        ...
        for payload in product_payloads:
            ...
            if response.status_code in [200, 201]:
                ...
                product_id = next((pid for pid, p in product_map.items() if p.name == product_name), None)
                ...
                if product.attribute_line_ids:
                    ...
                    if variant_response.status_code == 200:
                        ...
                        if data_list:
                            ...
                            if prod_var:
                                prod_var.write({
                                    'bigcommerce_product_attribute_id': variant_data.get('id'),
                                    'bigcommerce_product_id': variant_data.get('product_id'),
                                    'bigcommerce_sku': variant_data.get('sku'),
                                    'bigcommerce_sku_id': variant_data.get('sku_id') or 0,
                                    'bigcommerce_store_id': self.id,
    ...
    def _process_bigcommerce_order(self, order_data, headers):
        ...
        for prod in prod_response.json():
            ...
            product = self.env['product.product'].browse(cr_product.get('id')) if cr_product.get('id') else None
            _logger.info('Yes Product : {product}')
            if product:
        ...
        odoo_state = 'sale' if order_data.get('status') in ['Awaiting Fulfillment','Awaiting Shipment','Awaiting Pickup','Partially Shipped','Shipped', 'Completed','Awaiting Payment'] else 'draft'
        ...
        if order_data.get('status') in ['Cancelled','Declined'] and sale_order.state != 'cancel':
        ...
        for store in self:
            ...
            try:
                ...
                if not response.content:
                    raise UserError("No data returned from BigCommerce (empty response).")
    ...
    def _update_bigcommerce_order(self, order_data, headers, sale_order):
        ...
        # Determine Odoo state
        odoo_state = 'sale' if order_data.get('status') in ['Awaiting Fulfillment', 'Awaiting Shipment',
                                                            'Awaiting Pickup', 'Partially Shipped', 'Shipped',
                                                            'Completed', 'Awaiting Payment'] else 'draft'
        ...
        if order_data.get('status') == 'Shipped':
            ...
            if picking:
                ...
                if shipped_items and any(move.product_uom_qty > 0 for move in picking.move_ids_without_package):
                    try:
                        picking.action_assign()
                        picking.button_validate()
                        self.env['cr.data.processing.log']._log_data_processing(
        ...
        if bc_status in ['Awaiting Fulfillment', 'Awaiting Shipment'] and sale_order.state == 'draft':
            sale_order.action_confirm()
    ...
    def _sync_tax_classes(self):
        ...
        for tc in tax_classes:
            ...
            if not tax:
                _logger.info(f'company :{self.env.company.id}')
                _logger.info(f'country_id :{self.env.company.country_id.id}')
        ...
        try:
            for item in items:
                ...
                product = self.env['product.product'].sudo().search([
                    ...
                    ('bigcommerce_product_attribute_id', '=', identity['variant_id']),
                ], limit=1)
                if not product:
    ...
    def delete_product_cron(self):
        ...
        self.ensure_one()
        cron = self.env['ir.cron'].sudo().search([('name', '=', "BigCommerce Product Import")], limit=1)
        if cron:
            cron.unlink()
            return {
                "type": "ir.actions.client",
    ...
    def delete_order_cron(self):
        ...
        self.ensure_one()
        cron = self.env['ir.cron'].sudo().search([('name', '=', "BigCommerce Order Import")], limit=1)
        if cron:
            cron.unlink()
            return {
                "type": "ir.actions.client",
