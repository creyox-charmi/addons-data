...
class ProductProduct(models.Model):
    ...
    def action_export_inventory_to_bigcommerce(self, store):
        ...
        data = self.env['product.product'].search([('bigcommerce_store_id','=',store.id)])
        ...
        for product in data:
            ...
            # Filter valid stock quants with location that has bc_location_id
            for data  in product.stock_quant_ids:
                print('data : ',data)

            valid_quants = product.stock_quant_ids.filtered(
                lambda q: q.location_id.usage == 'internal' and q.location_id.bc_location_id
            )

            quantity = sum(valid_quants.mapped('quantity'))

            if not valid_quants or quantity <= 0:
                _logger.warning("No valid stock quant or zero quantity for product %s", product.name)
                continue
