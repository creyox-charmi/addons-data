...
class BigCommerceStore(models.Model):
    ...
    currency_symbol_location = fields.Selection([
        ('left', 'Left'),
        ('right', 'Right')
    ], string="Currency Symbol Location")
    ...
    active_comparison_modules = fields.Char(
        string="Active Comparison Modules")
    connection_status = fields.Selection([
        ('draft', 'Draft'),
        ('fail', 'Failed'),
        ('connected', 'Connected')
    ], string="Connection Status", default='draft')
    product_category_ids = fields.One2many('product.category', 'bigcommerce_store_id', string='Product Categories')
    product_template_ids = fields.One2many('product.template', 'bigcommerce_store_id', string='Products')
    partner_ids = fields.One2many('res.partner', 'bigcommerce_store_id', string='Customers')
    sale_order_ids = fields.One2many('sale.order', 'bigcommerce_store_id', string='Orders')
    ...
    # domain=[('can_be_sold', '=', True), ('type', '=', 'service')])
    webhook_ids = fields.One2many('bigcommerce.webhook', 'bigcommerce_store_id', string='Webhooks')
    ...
    import_cron_value = fields.Integer(
    ...
    import_cron_value1 = fields.Integer(
        string="Import Cron Value",
    )
    ...
    cr_logs_ids = fields.One2many(
        "cr.data.processing.log", "cr_shop_id", string="Logs"
    )
    ...
    total_customers = fields.Integer(string="Total Customers", compute="_compute_totals", store=True)
    ...
    total_shipped_orders = fields.Integer(string="Shipped Orders", compute="_compute_totals", store=True)
    total_refunded_orders = fields.Integer(string="Refunded Orders", compute="_compute_totals", store=True)
    sales_chart_data = fields.Char(string="Chart Data", compute="_compute_sales_chart_data")
    sales_chart_points = fields.Char(string="Chart Points", compute="_compute_sales_chart_data")
    sales_chart_points = fields.Char(string="Chart Points", compute="_compute_sales_chart_points")
    ...
    def _compute_sales_chart_points(self):
        for rec in self:
            orders = rec.sale_order_ids.sorted('date_order', reverse=True)[:5]
            values = [int(o.amount_total or 0) for o in orders]
            points = ' '.join(f'{i * 25},{40 - y}' for i, y in enumerate(values))
            rec.sales_chart_points = points

    def _compute_sales_chart_data(self):
        for rec in self:
            orders = rec.sale_order_ids.sorted('date_order', reverse=True)[:5]
            amounts = [int(o.amount_total or 0) for o in orders]
            rec.sales_chart_data = ','.join(map(str, amounts))
            rec.sales_chart_points = ' '.join(f'{i * 25},{40 - int(y)}' for i, y in enumerate(amounts))
    ...
    def _compute_totals(self):
        for record in self:
            ...
            record.total_shipped_orders = len(record.sale_order_ids.filtered(lambda o: o.state == 'done'))
            record.total_refunded_orders = len(record.sale_order_ids.filtered(
                lambda o: o.invoice_status == 'invoiced' and any(
                    inv.move_type == 'out_refund' for inv in o.invoice_ids)))

    def _compute_sales_chart_data(self):
        for rec in self:
            # Example: last 5 orders totals
            last_orders = rec.sale_order_ids.sorted('date_order', reverse=True)[:5]
            amounts = [int(o.amount_total) for o in last_orders]
            rec.sales_chart_data = ','.join(map(str, amounts))
    ...
    def action_sync_store_info(self):
        ...
        fail = 0
        for record in self:
            # 🚫 Skip if store is not in the current company
            ...
            if not all(
                    [record.client_id, record.client_secret, record.access_token, record.api_url, record.store_hash]):
                raise UserError(_("Please fill in Client ID, Client Secret, Access Token, API URL, and Store Hash."))
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    record.write({
                        ...
                        'control_panel_base_url': data.get('control_panel_base_url'),
                        ...
                        'currency_symbol_location': data.get('currency_symbol_location'),

                        'weight_units': data.get('weight_units'),
                        'dimension_units': data.get('dimension_units'),
                        'dimension_decimal_places': data.get('dimension_decimal_places'),
                        'dimension_decimal_token': data.get('dimension_decimal_token'),
                        'dimension_thousands_token': data.get('dimension_thousands_token'),
                        ...
                        'logo_url': data.get('logo')[0].get('url') if isinstance(data.get('logo'), list) and data.get(
                            'logo') else '',
                        'is_price_entered_with_tax': data.get('is_price_entered_with_tax'),

                        'store_id': data.get('store_id'),
                        'default_site_id': data.get('default_site_id'),
                        'default_channel_id': data.get('default_channel_id'),
                        'active_comparison_modules': json.dumps(data.get('active_comparison_modules', [])),
                    ...
                    raise UserError(_("Failed to connect to BigCommerce API. Status Code: %s\nResponse: %s")
    ...
    def import_bigcommerce_products(self):
        ...
        for store in self:
            ...
            if response.status_code == 200:
                ...
                shop_all_category_id = self.env['product.category'].search([
                ...
                for item in data:
                    ...
                    # Match all valid categories, excluding Shop All
                    ...
                    # Step 1: Get the brand_id from product data
                    brand_id = item.get('brand_id')

                    # Step 2: Find the brand record in Odoo
                    brand_rec = self.env['bigcommerce.brand'].search([
                        ('brand_id', '=', brand_id),
                        ('store_id', '=', store.id)  # optional if you store brand per store
                    ], limit=1)
                ...
                self.env['cr.data.processing.log'].sudo()._log_data_processing(
                    cr_shop_id=self.id,
                    cr_message="Sucessfully fetched products",
                    record_count=count,
                    status='success',
                    timespan=str(fields.Datetime.now()),
                    initiated_at=str(fields.Datetime.now()),
                ...
                return {
                    ...
                    'params': {
                        ...
                        'message': _("Products Import Successfully."),
                        'sticky': False,
                        'next': {
                            'type': 'ir.actions.client',
                            'tag': 'soft_reload'
                ...
                self.env['cr.data.processing.log'].sudo()._log_data_processing(
                    cr_shop_id=self.id,
                    cr_message="Unsucessfully to fetched products",
                    record_count=fail,
                    status='failure',
                    timespan=str(fields.Datetime.now()),
                    initiated_at=str(fields.Datetime.now()),
                ...
                raise UserError(f"Failed to fetch products: {response.status_code}")
        return None
    ...
    def action_import_bigcommerce_customers(self):
        ...
        for store in self:
            ...
            try:
                ...
                self.env['cr.data.processing.log'].sudo()._log_data_processing(
                    cr_shop_id=self.id,
                    cr_message="Sucessfully fetched customers",
                    record_count=count,
                    status='success',
                    timespan=str(fields.Datetime.now()),
                    initiated_at=str(fields.Datetime.now()),
                ...
                return {
                    ...
                    'params': {
                        'type': 'success',
                        'title': _("Import Successfully"),
                        'message': _("Customers imported successfully."),
                        'sticky': False,
                ...
                self.env['cr.data.processing.log'].sudo()._log_data_processing(
                    cr_shop_id=self.id,
                    cr_message="Unsucessfull to fetched customer",
                    record_count=fail,
                    status='failure',
                    timespan=str(fields.Datetime.now()),
                    initiated_at=str(fields.Datetime.now()),
                    error_message=f"Failed to import customers:\n{str(e)}",
                    cr_user_id=self.env.uid
                )
                raise UserError(f"Failed to import customers:\n{str(e)}")
        return None
        ...
        for store in self:
            ...
            try:
                ...
                for addr in addresses:
                    ...
                    # Get country and state records
                    country = self.env['res.country'].search([
                        '|', ('code', '=', addr.get('country_code')), ('name', '=', addr.get('country'))
                    ], limit=1)
                    state = self.env['res.country.state'].search([
                        ('name', 'ilike', addr.get('state_or_province', '')),
                ...
                self.env['cr.data.processing.log'].sudo()._log_data_processing(
                    cr_shop_id=self.id,
                    cr_message="Sucessfully fetched customers address",
                    record_count=count,
                    status='success',
                    timespan=str(fields.Datetime.now()),
                    initiated_at=str(fields.Datetime.now()),
                ...
                return {
                    ...
                    'params': {
                        ...
                        'message': _("Customer addresses updated successfully."),
                        'sticky': False,
                    }
                }
                ...
                self.env['cr.data.processing.log'].sudo()._log_data_processing(
                    cr_shop_id=self.id,
                    cr_message="Unsucessfull to fetched customer address",
                    record_count=fail,
                    status='failure',
                    timespan=str(fields.Datetime.now()),
                    initiated_at=str(fields.Datetime.now()),
                    error_message=f"Failed to import customer addresses:\n{str(e)}",
                    cr_user_id=self.env.uid
                )
                raise UserError(f"Failed to import customer addresses:\n{str(e)}")
        return None

    # def action_import_bigcommerce_orders(self):
    #     """Import orders from BigCommerce to Odoo, handling shipping via delivery.carrier."""
    #     for store in self:
    #         if not store.access_token or not store.store_hash:
    #             raise UserError("Missing API credentials on the store record.")
    #
    #         headers = {
    #             "X-Auth-Token": store.access_token,
    #             "Accept": "application/json",
    ...
    #             raise UserError(f"Failed to fetch orders: {str(e)}")
    #
    ...
    #             if self.env['sale.order'].sudo().search([('bigcommerce_order_id', '=', order['id'])], limit=1):
    #                 _logger.info("Order %s already imported, skipping", order['id'])
    #                 continue
    #
    #             # Find customer
    #             customer = self.env['res.partner'].sudo().search([
    #                 ('bigcommerce_customer_id', '=', order['customer_id'])
    #             ], limit=1)
    #             if not customer:
    #                 _logger.warning("Customer %s not found for order %s, skipping", order['customer_id'], order['id'])
    #                 continue
    #
    #             # Get product lines
    #             products_url = order.get('products', {}).get('url')
    #             if not products_url:
    #                 _logger.warning("No products URL for order %s, skipping", order['id'])
    #                 continue
    #
    #             try:
    ...
    #             except requests.exceptions.RequestException as e:
    #                 _logger.error("Failed to fetch products for order %s: %s", order['id'], str(e))
    #                 continue
    #
    #             order_lines = []
    ...
    #
    #                 if not product:
    #                     _logger.warning("Product %s (variant %s) not found for order %s, skipping line",
    #                                     prod['product_id'], prod['variant_id'], order['id'])
    #                     continue
    #
    #                 order_lines.append((0, 0, {
    #                     'product_id': product.id,
    #                     'product_uom_qty': prod['quantity'],
    #                     'price_unit': float(prod['base_price']),  # Excl. tax
    #                     'tax_id': [(6, 0, [])],  # Taxes in BigCommerce fields
    #                 }))
    #
    #             if not order_lines:
    #                 _logger.warning("No valid order lines for order %s, skipping", order['id'])
    #                 continue
    #
    #             # Get shipping details
    #             shipping_addresses = []
    #             shipping_addresses_data = order.get('shipping_addresses', {})
    #             if isinstance(shipping_addresses_data, dict) and 'url' in shipping_addresses_data:
    #                 try:
    #                     addr_response = requests.get(shipping_addresses_data['url'], headers=headers)
    #                     addr_response.raise_for_status()
    #                     shipping_addresses = addr_response.json()
    #                 except requests.exceptions.RequestException as e:
    #                     _logger.error("Failed to fetch shipping addresses for order %s: %s", order['id'], str(e))
    #
    ...
    #                 shipping = shipping_addresses[0]  # Use first address
    #                 # Find shipping zone
    #                 zone = self.env['bigcommerce.shipping.zone'].sudo().search([
    #                     ('bigcommerce_store_id', '=', store.id),
    #                     ('country_iso2', '=', shipping.get('country_iso2')),
    #                 ], limit=1)
    #
    #                 # Find inventory location (if provided)
    #                 location_id = shipping.get('location_id')  # Adjust if in shipment data
    #                 if location_id:
    ...
    #                     ], limit=1)
    #             else:
    #                 _logger.info("No shipping addresses for order %s, proceeding without zone or location", order['id'])
    #
    #             # For shipped orders, fetch shipment details
    #             if order.get('status') == 'Shipped':
    #                 shipment_url = f"https://api.bigcommerce.com/stores/{store.store_hash}/v2/orders/{order['id']}/shipments"
    #                 try:
    #                     shipment_response = requests.get(shipment_url, headers=headers)
    #                     _logger.info(f"shipment_response {shipment_response.status_code}")
    ...
    #                                 ('store_id', '=', store.id),
    #                                 ('name', 'ilike', shipment['shipping_method']),
    #                             ], limit=1)
    #                             if not carrier:
    #                                 carrier = self.env['delivery.carrier'].sudo().search([
    #                                     ('delivery_type', '=', 'bigcommerce'),
    #                                     ('store_id', '=', store.id),
    #                                     ('name', 'ilike', shipment['shipping_provider_display_name']),
    #                                 ], limit=1)
    #
    #                             tracking_number = shipment.get('tracking_number')
    #                             shipped_items = shipment.get('items', [])
    #                 except requests.exceptions.RequestException as e:
    #                     _logger.error("Failed to fetch shipments for order %s: %s", order['id'], str(e))
    #
    #             # Validate shipping cost against carrier settings
    #             final_shipping_cost = shipping_cost
    #             if carrier and shipping_cost > 0:
    #                 subtotal = float(order.get('subtotal_ex_tax', 0))
    #                 if carrier.use_discounted_sub_total:
    #                     subtotal -= float(order.get('discount_amount', 0))
    #                 if carrier.minimum_sub_total and subtotal < carrier.minimum_sub_total:
    #                     _logger.warning("Order %s subtotal (%s) below minimum (%s), skipping shipping",
    #                                     order['id'], subtotal, carrier.minimum_sub_total)
    #                     final_shipping_cost = 0
    #                 else:
    #                     final_shipping_cost += carrier.fixed_surcharge
    #
    #             # Find status
    #             status = self.env['bigcommerce.order.status'].sudo().search([
    #                 ('bigcommerce_status_id', '=', order.get('status_id')),
    #                 ('store_id', '=', store.id)
    #             ], limit=1)
    #
    #             # Determine Odoo state
    #             odoo_state = 'sale' if order.get('status') in ['Shipped', 'Completed'] else 'draft'
    #
    #             # Parse date
    #             try:
    #                 date_order = datetime.strptime(order['date_created'], '%a, %d %b %Y %H:%M:%S %z').replace(
    #                     tzinfo=None)
    #             except ValueError:
    #                 date_order = fields.Datetime.now()
    #                 _logger.warning("Invalid date format for order %s, using current time", order['id'])
    #
    #             # Create Sale Order
    #             sale_order_vals = {
    ...
    #                 'bigcommerce_store_id': store.id,
    #                 'bigcommerce_order_id': order['id'],
    #                 'bigcommerce_order_status_id': status.id if status else False,
    #                 'bigcommerce_subtotal_ex_tax': float(order.get('subtotal_ex_tax', 0)),
    #                 'bigcommerce_subtotal_inc_tax': float(order.get('subtotal_inc_tax', 0)),
    #                 'bigcommerce_subtotal_tax': float(order.get('subtotal_tax', 0)),
    #                 'bigcommerce_base_shipping_cost': float(order.get('base_shipping_cost', 0)),
    #                 'bigcommerce_shipping_cost_ex_tax': float(order.get('shipping_cost_ex_tax', 0)),
    #                 'bigcommerce_shipping_cost_inc_tax': float(order.get('shipping_cost_inc_tax', 0)),
    #                 'bigcommerce_total_ex_tax': float(order.get('total_ex_tax', 0)),
    #                 'bigcommerce_total_inc_tax': float(order.get('total_inc_tax', 0)),
    #                 'bigcommerce_total_tax': float(order.get('total_tax', 0)),
    #                 'bigcommerce_payment_method': order.get('payment_method', ''),
    #                 'bigcommerce_payment_status': order.get('payment_status', ''),
    #                 'bigcommerce_refunded_amount': float(order.get('refunded_amount', 0)),
    #                 'bigcommerce_discount_amount': float(order.get('discount_amount', 0)),
    #                 'bigcommerce_customer_message': order.get('customer_message', ''),
    #                 'bigcommerce_staff_notes': order.get('staff_notes', ''),
    ...
    #                 sale_order._set_delivery_cost(carrier, final_shipping_cost)
    #                 _logger.info("Set delivery cost %s for order %s with carrier %s",
    #                              final_shipping_cost, order['id'], carrier.name)
    #
    #             # Confirm sale order if status is 'sale'
    #             # if odoo_state == 'sale':
    ...
    #                                 _logger.info(f"move.quantity {move.quantity} {move}")
    #                     # Validate picking only if quantities are set or reserved
    #                     if shipped_items and any(move.quantity > 0 for move in picking.move_ids_without_package):
    #                         try:
    #                             picking.action_assign()
    #                             picking.button_validate()
    #                             _logger.info(
    #                                 "Created and validated delivery for order %s from location %s with tracking %s",
    #                                 order['id'], location.name if location else 'default', tracking_number or 'none')
    #                         except Exception as e:
    #                             _logger.error("Failed to validate delivery for order %s: %s", order['id'], str(e))
    #                     else:
    #                         _logger.warning(
    #                             "No quantities reserved or set for delivery of order %s, skipping validation",
    #                             order['id'])
    #
    #         return {
    #             'type': 'ir.actions.client',
    ...
    #                 'message': _("Successfully imported %d sale order(s)." % imported_count),
    #                 'sticky': False,
    #             }
    #         }
    ...
    def action_import_order_statuses(self):
        ...
        if response.status_code != 200:
            ...
            self.env['cr.data.processing.log'].sudo()._log_data_processing(
                cr_shop_id=self.id,
                cr_message="Unsucessfull to fetched order status",
                record_count=fail,
                status='failure',
                timespan=str(fields.Datetime.now()),
                initiated_at=str(fields.Datetime.now()),
                error_message=f"Failed to import order statuse. Response:\n{response.text}",
                cr_user_id=self.env.uid
            )
            raise UserError(_("Failed to fetch order statuses from BigCommerce. Response: %s") % response.text)

        order_statuses = response.json()
        status_model = self.env['bigcommerce.order.status']

        for status in order_statuses:
            exist = status_model.search([('bigcommerce_status_id','=',status.get('id')),('store_id','=',self.id)])
            if exist:
                status_model.write({
                    'store_id': self.id,
        ...
        self.env['cr.data.processing.log'].sudo()._log_data_processing(
            cr_shop_id=self.id,
            cr_message="Sucessfully fetched Order status",
            record_count=count,
            status='success',
            timespan=str(fields.Datetime.now()),
            initiated_at=str(fields.Datetime.now()),
        ...
        return {
            ...
            'params': {
                ...
                'message': _("Order status records were successfully imported from BigCommerce."),
                'sticky': False,
            }
        }
    ...
    def action_import_bigcommerce_brands(self):
        ...
        for store in self:
            ...
            if response.status_code != 200:
                ...
                self.env['cr.data.processing.log'].sudo()._log_data_processing(
                    cr_shop_id=self.id,
                    cr_message="Unsucessfull to fetched brand.",
                    record_count=fail,
                    status='failure',
                    timespan=str(fields.Datetime.now()),
                    initiated_at=str(fields.Datetime.now()),
            ...
            self.env['cr.data.processing.log'].sudo()._log_data_processing(
                cr_shop_id=self.id,
                cr_message="Sucessfully fetched brand",
            ...
            return {
                ...
                'params': {
                    ...
                    'message': _("Successfully imported %d brand(s)." % imported_count),
                    'sticky': False,
                }
            }
        return None
    ...
    #     imported_count = 0
    #
    #     currency_obj = self.env['bigcommerce.currency']
    #
    ...
    #
    #         vals = {
    #             'bigcommerce_currency_id':curr.get('id'),
    #             'store_id': self.id,
    #             'currency_code': curr.get('currency_code'),
    #             'name': curr.get('name'),
    #             'enabled': curr.get('enabled'),
    #             'is_transactional': curr.get('is_transactional'),
    #             'is_default': curr.get('is_default'),
    #             'auto_update': curr.get('auto_update'),
    #             'currency_exchange_rate': curr.get('currency_exchange_rate'),
    #             'token': curr.get('token'),
    #             'token_location': curr.get('token_location'),
    #             'decimal_places': curr.get('decimal_places'),
    #             'decimal_token': curr.get('decimal_token'),
    #             'thousands_token': curr.get('thousands_token'),
    #             'default_for_country_codes': country_codes,
    #             'country_iso2': curr.get('country_iso2'),
    #             'last_updated': last_updated,
    #             'use_default_name': curr.get('use_default_name'),
    #         }
    #
    ...
    def action_import_bigcommerce_currencies(self):
        ...
        for curr in currencies:
            ...
            print('existing_currency : ', existing_currency)
            if existing_currency:
                existing_currency.write(vals)
            else:
                vals['name'] = name
    ...
    #             "last_name": customer.name.split()[-1] if customer.name else "Customer",
    #             "company": customer.bigcommerce_company or "",
    #             "phone": customer.phone or "",
    #             "notes": customer.bigcommerce_notes or "",
    #             "tax_exempt_category": customer.bigcommerce_tax_exempt_category or "",
    #             "customer_group_id": customer.bigcommerce_group_id or 0,
    #             "addresses": [
    #                 {
    #                     "address1": customer.street or "Unknown Street",
    #                     "city": customer.city or "Unknown City",
    #                     "country_code": customer.country_id.code if customer.country_id else "US",
    #                     "first_name": customer.name.split()[0] if customer.name else "Unknown",
    #                     "last_name": customer.name.split()[-1] if customer.name else "Customer",
    #                     "phone": customer.phone or "",
    #                     "postal_code": customer.zip or "00000",
    #                     "state_or_province": customer.state_id.name if customer.state_id else "",
    #                 }
    #             ],
    ...
    #         data = response.json().get('data', [])
    #         for cust_data, partner in zip(data, customers):
    #             partner.write({
    #                 'bigcommerce_store_id': self.id,
    ...
    #                 'message': _("Customers were successfully exported to BigCommerce."),
    #                 'type': 'success',
    #                 'sticky': False,
    #             }
    #         }
    ...
    def action_export_customers_to_bigcommerce(self):
        ...
        return {
            ...
            'params': {
                'title': _("Export Success"),
                'message': _(f"Exported {total} customers to BigCommerce successfully."),
                'type': 'success',
                'sticky': False,
            }
        }
    ...
    def action_export_customer_addresses(self):
        ...
        if not customers:
            ...
            raise UserError("No customers with BigCommerce customer IDs found to export addresses.")
        ...
        for customer in customers:
            ...
            addresses = customer.child_ids.filtered(
                lambda a: a.type in ('contact', 'delivery', 'invoice'))  # or any logic to pick addresses
            for addr in addresses:
                first_name, last_name = payment_utils.split_partner_name(addr.name)
                address_data = {
                    "customer_id": customer.bigcommerce_customer_id,
                    "first_name": first_name or customer.name.split()[0] if customer.name else "Unknown",
                    "last_name": last_name or customer.name.split()[-1] if customer.name else "Customer",
        ...
        for i in range(batches):
            ...
            if response.status_code not in [200, 201]:
                ...
                raise UserError(f"Failed to export addresses batch {i + 1}: {response.text}")
    ...
    def action_export_brands_to_bigcommerce(self):
        ...
        for brand in brands:
            ...
            if response.status_code in [200, 201]:
                ...
                _logger.warning("Failed to export brand '%s': %s", brand.name, response.text)
    ...
    #     """Export product categories to BigCommerce and update fields from API response."""
    #     self.ensure_one()
    #     count = 0
    #     fail = 0
    #     if not self.access_token or not self.store_hash:
    #         self.env['cr.data.processing.log'].sudo()._log_data_processing(
    #             cr_shop_id=self.id,
    #             cr_message="Unsucessfull to export categories.",
    #             record_count=fail,
    #             status='failure',
    #             timespan=str(fields.Datetime.now()),
    #             initiated_at=str(fields.Datetime.now()),
    ...
    #
    #
    #     categories = self.env['product.category'].search([('bigcommerce_category_id', '=', None)])
    ...
    #             timespan=str(fields.Datetime.now()),
    #             initiated_at=str(fields.Datetime.now()),
    #             error_message=f"No categories to export.",
    #             cr_user_id=self.env.uid
    #         )
    ...
    #     }
    #
    #     url_for_tree = f"https://api.bigcommerce.com/stores/{self.store_hash}/v3/catalog/trees"
    #     response = requests.get(url_for_tree, headers=headers)
    #     response_data = response.json()
    #
    #     # Get the first tree ID
    #     tree_id = response_data.get("data", [])[0].get("id") if response_data.get("data") else None
    #
    ...
    #
    #         if not (category.bigcommerce_tree_id or category.bigcommerce_parent_id):
    #             continue
    #
    #         cat_payload = {
    #             "name": category.name,
    #         }
    #
    #         if category.bigcommerce_tree_id:
    #             cat_payload["tree_id"] = category.bigcommerce_tree_id
    #         else:
    #             cat_payload["parent_id"] = category.bigcommerce_parent_id
    #
    ...
    #
    #         cat_payload["is_visible"] = category.is_visible
    #
    ...
    #             }
    #
    #         payload_list.append(cat_payload)
    #
    #     if not payload_list:
    #         self.env['cr.data.processing.log'].sudo()._log_data_processing(
    #             cr_shop_id=self.id,
    ...
    #             timespan=str(fields.Datetime.now()),
    #             initiated_at=str(fields.Datetime.now()),
    #             error_message=f"No valid categories to export.",
    #             cr_user_id=self.env.uid
    #         )
    #         raise UserError(_("No valid categories to export."))
    #
    #     response = requests.post(url, headers=headers, json=payload_list)
    #     print('response.status_code : ',response.status_code)
    #     print('response.json : ',response.json())
    #     if response.status_code != 201:
    #         raise UserError(_("Failed to export categories. %s") % response.text)
    #
    #     response_data = response.json().get("data", [])
    #
    #     for data in response_data:
    #         category = categories.filtered(lambda c: c.name == data.get("name"))
    #         if not category:
    #             continue
    #
    ...
    #     self.env['cr.data.processing.log'].sudo()._log_data_processing(
    #         cr_shop_id=self.id,
    #         cr_message="Sucessfully exported categories",
    #         record_count=count,
    #         status='success',
    #         timespan=str(fields.Datetime.now()),
    #         initiated_at=str(fields.Datetime.now()),
    ...
    #             'message': _("Successfully exported %d categories to BigCommerce.") % len(response_data),
    #             'type': 'success',
    #             'sticky': False,
    #         }
    #     }
    ...
    def action_export_categories_to_bigcommerce(self):
        ...
        categories = self.env['product.category'].search([('bigcommerce_category_id', '=', 0)])
        print('categories : ', categories)
        ...
        for level in sorted(level_map.keys()):
            ...
            print(f'Level {level} response.status_code : ', response.status_code)
            print(f'Level {level} response.json : ', response.json())
            if response.status_code != 201:
                self.env['cr.data.processing.log'].sudo()._log_data_processing(
                    cr_shop_id=self.id,
                ...
                raise UserError(_("Failed to export categories at level %d. %s") % (level, response.text))
        ...
        return {
            ...
            'params': {
                'title': _("Export Complete"),
                'message': _("Successfully exported %d categories to BigCommerce.") % count,
                'type': 'success',
                'sticky': False,
            }
        }
    ...
    #
    #     payload_list = []
    #     product_map = {}  # To track product name -> record
    #
    ...
    #         if response.status_code in [200, 201]:
    #             data = response.json().get('data', {})
    #             product = product_map.get(data.get("name"))
    #             if product:
    #                 product.bigcommerce_product_id = data.get("id")
    #                 created_count += 1
    #         else:
    #             _logger.warning("Failed to export product: %s\nResponse: %s", payload.get("name"), response.text)
    #
    #     return {
    #         'type': 'ir.actions.client',
    #         'tag': 'display_notification',
    #         'params': {
    #             'title': _("Export Complete"),
    #             'message': _("Successfully exported %d products to BigCommerce.") % created_count,
    #             'type': 'success',
    #             'sticky': False,
    #         }
    #     }
    ...
    def upload_bigcommerce_product_image(self, product, store_hash, access_token):
        ...
        #         timespan=str(fields.Datetime.now()),
        #         initiated_at=str(fields.Datetime.now()),
        #         error_message=f"Product {product.name} has no BigCommerce product ID."
        #     )
        #     _logger.warning("Product %s has no BigCommerce product ID.", product.name)
        #     return
        ...
        files = {
            'image_file': ('product_image.jpg', image_file, mimetype),
        }
        ...
        if response.status_code in [200, 201]:
            ...
            _logger.info("✅ Image uploaded for product: %s", product.name)
        else:
            fail += 1
            self.env['cr.data.processing.log'].sudo()._log_data_processing(
                cr_shop_id=self.id,
            ...
            _logger.error("❌ Failed to upload image for product %s. Status: %s, Response: %s",
    ...
    def action_export_products_to_bigcommerce(self):
        ...
        if not products:
            self.env['cr.data.processing.log'].sudo()._log_data_processing(
                ...
                timespan=str(fields.Datetime.now()),
                initiated_at=str(fields.Datetime.now()),
                error_message=f"No products to export.",
                cr_user_id=self.env.uid
            )
            raise UserError(_("No products to export."))
        ...
        for product in products:
            ...
            payload = {
                ...
                "calculated_price": product.bigcommerce_calculated_price or None,
                "categories": category_ids,
                "brand_id": product.bigcommerce_brand_id.brand_id or None,
                "option_set_id": product.bigcommerce_option_set_id or None,
                "inventory_level": product.bigcommerce_inventory_level or int(product.qty_available),
                "inventory_tracking": product.bigcommerce_tracking or "product",
                "layout_file": product.bigcommerce_layout_file or "",
        ...
        if not payload_list:
            self.env['cr.data.processing.log'].sudo()._log_data_processing(
                ...
                timespan=str(fields.Datetime.now()),
                initiated_at=str(fields.Datetime.now()),
                error_message=f"No valid product data to export.",
                cr_user_id=self.env.uid
            )
            raise UserError(_("No valid product data to export."))
        ...
        for payload in payload_list:
            ...
            if response.status_code in [200, 201]:
                ...
                _logger.info(f'res_data : {res_data}')
                ...
                if product.attribute_line_ids:
                    for attribute_line in product.attribute_line_ids:
                        product.create_bigcommerce_product_option(product, attribute_line.attribute_id)
                ...
                # if product:
                #     product.bigcommerce_product_id = data.get("id")
                #     self.upload_bigcommerce_product_image(product, self.store_hash, self.access_token)
                #     created_count += 1
            else:
                ...
                self.env['cr.data.processing.log'].sudo()._log_data_processing(
                    ...
                    timespan=str(fields.Datetime.now()),
                    initiated_at=str(fields.Datetime.now()),
                    error_message=f"Failed to export product: {name} Response : {response.text}",
                    cr_user_id=self.env.uid
                )
                _logger.warning("Failed to export product: %s\nResponse: %s", payload.get("name"), response.text)
    ...
    def action_import_inventory_locations(self):
        ...
        for location in data:
            ...
            parent = InventoryLocation.search([
                ('name', '=', 'WH'),
            ], limit=1)
        ...
        return {
            ...
            'params': {
                ...
                'message': _("Successfully imported %d inventory location(s) from BigCommerce.") % created_count,
                'type': 'success',
                'sticky': False,
            }
        }
    ...
    #     url = f"https://api.bigcommerce.com/stores/{self.store_hash}/v3/inventory/locations"
    #
    #     locations = self.env['bigcommerce.inventory.location'].search([
    #         ('bigcommerce_store_id', '=', self.id),
    #         ('bc_location_id','=',None)
    #     ])
    #     print('locations : ',locations)
    #     VALID_US_STATE_CODES = [
    #         'AL', 'AK', 'AS', 'AZ', 'AR', 'AE', 'AA', 'AP', 'CA', 'CO', 'CT', 'DE', 'DC', 'FM', 'FL', 'GA', 'GU', 'HI',
    #         'ID', 'IL', 'IN', 'IA',
    ...
    #         location_data = {
    #             "code": loc.code or f"LOC-{loc.id}",
    #             "label": loc.label or "No Label",
    ...
    #             'params': {
    #                 'title': _("Export Complete"),
    #                 'message': _("Successfully exported %d locations to BigCommerce.") % len(payload),
    #                 'type': 'success',
    #                 'sticky': False,
    #             }
    #         }
    #     else:
    #         raise UserError(_("Failed to export locations. Response: %s") % response.text)
    ...
    def action_export_locations_to_bigcommerce(self):
        ...
        def to_bool(val):
            return bool(val) if isinstance(val, bool) else str(val).strip().lower() in ('1', 'true', 'yes')
        ...
        if response.status_code in [200, 201]:
            ...
            print('created_locations : ', created_locations)
    ...
    def action_import_shipping_zones(self):
        ...
        if response.status_code != 200:
            ...
            raise UserError(_("Failed to fetch shipping zones. Status code: %s\nResponse: %s") %
                            (response.status_code, response.text))
        ...
        for zone in data:
            ...
            zone_vals = {
                ...
                'free_shipping_minimum_sub_total': float(zone['free_shipping']['minimum_sub_total']),
                'exclude_fixed_shipping_products': zone['free_shipping']['exclude_fixed_shipping_products'],
                ...
                for method in methods_data:
                    ...
                    method_vals = {
                        ...
                        'minimum_sub_total': float(
                            method.get('settings', {}).get('carrier_options', {}).get('minimum_sub_total', 0))
                        if method['type'] == 'freeshipping' else 0,
                        'rate': float(method.get('settings', {}).get('rate', 0)) if method['type'] == 'perorder' else 0,
                        'channel_ids_text': ','.join(str(cid) for cid in method.get('channel_ids', [])),
                        'store_id': self.id,

                    }
                    print('method_vals :  ', method_vals)
                    ...
                    if method_exist:
                        ...
                        name = method['name']
                        cat = self.env['product.category'].search([('name', '=', 'Deliveries')])
        ...
        self.env['cr.data.processing.log'].sudo()._log_data_processing(
            cr_shop_id=self.id,
            cr_message="Sucessfully fetched shipping zones",
        ...
        return {
            ...
            'params': {
                'title': _("Import Complete"),
                'message': _("Successfully imported %d shipping zones.") % created,
                'type': 'success',
                'sticky': False,
            }
        }
    ...
    def _get_webhook_destination(self):
        ...
        base_url = self.env['ir.config_parameter'].get_param(
            'web.base.url', '')
        _logger.info(f"base_url>>>>>>>> {base_url} ")
        ...
        if parsed_url.scheme != 'https':
            ...
            raise UserError("Webhook destination must use HTTPS. Please configure 'web.base.url' with an HTTPS URL.")
    ...
    def action_delete_order_refunded_webhook(self):
        ...
        self.ensure_one()
        webhook = self.webhook_ids.filtered(lambda w: w.scope == 'store/order/refunded/created')
        if not webhook:
            self.env['cr.data.processing.log'].sudo()._log_data_processing(
                cr_shop_id=self.id,
    ...
    def action_delete_product_created_webhook(self):
        ...
        self.ensure_one()
        webhook = self.webhook_ids.filtered(lambda w: w.scope == 'store/product/created')
        if not webhook:
            self.env['cr.data.processing.log'].sudo()._log_data_processing(
                cr_shop_id=self.id,
    ...
    def action_delete_product_updated_webhook(self):
        ...
        self.ensure_one()
        webhook = self.webhook_ids.filtered(lambda w: w.scope == 'store/product/updated')
        if not webhook:
            self.env['cr.data.processing.log'].sudo()._log_data_processing(
                cr_shop_id=self.id,
    ...
    def action_create_product_inventory_updated_webhook(self):
        ...
        try:
            ...
            response.raise_for_status()
            webhook_data = response.json().get('data', {})
            _logger.info("webhook_data: %s", webhook_data)
            ...
            raise UserError(f"Failed to create product inventory updated webhook: {str(e)}")

    def action_delete_product_inventory_updated_webhook(self):
        """Delete the webhook for store/product/inventory/updated event."""
        self.ensure_one()
        webhook = self.webhook_ids.filtered(lambda w: w.scope == 'store/product/inventory/updated')
        if not webhook:
            self.env['cr.data.processing.log'].sudo()._log_data_processing(
                cr_shop_id=self.id,
    ...
    def action_create_sku_inventory_updated_webhook(self):
        """Create a webhook for store/sku/inventory/updated event (variant products) and store its data."""
        self.ensure_one()
        if not self.access_token or not self.store_hash:
            self.env['cr.data.processing.log'].sudo()._log_data_processing(
                cr_shop_id=self.id,
                cr_message="Unsucessfull to create 'SKU inventory updated webhook'",
                record_count=0,
                status='failure',
                timespan=str(fields.Datetime.now()),
                initiated_at=str(fields.Datetime.now()),
        ...
        try:
            ...
            self.env['cr.data.processing.log'].sudo()._log_data_processing(
                cr_shop_id=self.id,
                cr_message="Sucessfully created 'SKU inventory updated webhook'",
                record_count=1,
                status='success',
                timespan=str(fields.Datetime.now()),
                initiated_at=str(fields.Datetime.now()),
            ...
            return {
                ...
                'params': {
                    'title': 'Success',
                    'message': 'SKU inventory updated webhook (variant) created and stored successfully.',
                    'type': 'success',
                    'sticky': False,
                }
            }
            ...
            self.env['cr.data.processing.log'].sudo()._log_data_processing(
                ...
                timespan=str(fields.Datetime.now()),
                initiated_at=str(fields.Datetime.now()),
                error_message=f"Failed to create SKU inventory updated webhook: {str(e)}",
                cr_user_id=self.env.uid
            )
            raise UserError(f"Failed to create SKU inventory updated webhook: {str(e)}")

    def action_delete_sku_inventory_updated_webhook(self):
        """Delete the webhook for store/sku/inventory/updated event (variant products)."""
        self.ensure_one()
        webhook = self.webhook_ids.filtered(lambda w: w.scope == 'store/sku/inventory/updated')
        if not webhook:
            self.env['cr.data.processing.log'].sudo()._log_data_processing(
                cr_shop_id=self.id,
                cr_message="Unsucessfull to delete 'SKU inventory updated webhook'",
                record_count=0,
                status='failure',
                timespan=str(fields.Datetime.now()),
                initiated_at=str(fields.Datetime.now()),
            ...
            return {
                ...
                'params': {
                    'title': 'Warning',
                    'message': 'No SKU inventory updated webhook found for this store.',
                    'type': 'warning',
                    'sticky': False,
                }
            }
    ...
    def _process_bigcommerce_order(self, order_data, headers):
        ...
        # ], limit=1)
        sale_order_query = f"""SELECT * FROM sale_order WHERE bigcommerce_order_id = %s AND bigcommerce_store_id = %s"""
        self.env.cr.execute(sale_order_query, (order_data['id'], self.id))
        sale_order = self.env.cr.fetchall()
        _logger.info(f'sale {sale_order}')
        _logger.info(f'order_data {order_data}')

        bc_status = order_data.get('status')
        if bc_status == 'Cancelled':
            if sale_order:
                if sale_order.state != 'cancel':
                    sale_order.action_cancel()
        if bc_status in ['Awaiting Fulfillment', 'Awaiting Shipment'] :
            if sale_order:
        ...
        # # Find customer
        # customer = self.env['res.partner'].sudo().search([
        #     ('bigcommerce_customer_id', '=', order_data['customer_id'])
        # ], limit=1)

        # customer_query = f"""SELECT * FROM res_partner WHERE bigcommerce_customer_id = %s AND bigcommerce_store_id = %s AND company_id in %s"""
        # self.env.cr.execute(customer_query, (order_data['customer_id'], self.id, (False,self.company_id.id)))
        customer_query = """
        ...
        """
        self.env.cr.execute(customer_query, (order_data['customer_id'], self.id, self.company_id.id))
        ...
        for row in result:
            ...
            _logger.info(f">> customer mapped: {customer_dict}")
        _logger.info(f">> customer : {customer}")
        if not result:
            error_msg = f"Customer {order_data['customer_id']} not found for order {order_data['id']}"
            self.env['cr.data.processing.log']._log_data_processing(
                cr_shop_id=self.id,
                record_count=1,
                cr_message="Failed to process order: Customer not found",
                status='failure',
                timespan=str(fields.Datetime.now()),
                initiated_at=str(fields.Datetime.now()),
            ...
            _logger.warning("Customer %s not found for order %s, skipping", order_data['customer_id'], order_data['id'])
            return False
        ...
        try:
            ...
            error_msg = f"Failed to fetch products for order {order_data['id']}: {str(e)}"
            self.env['cr.data.processing.log']._log_data_processing(
                cr_shop_id=self.id,
                record_count=1,
            ...
            _logger.error("Failed to fetch products for order %s: %s", order_data['id'], str(e))
            return False
        ...
        for prod in prod_response.json():
            ...
            product_query = """
                        ...
                        WHERE bigcommerce_product_id = %s 
                          ...
                          AND bigcommerce_store_id = %s
                    """
            # product_query = f"""SELECT * FROM product_product WHERE bigcommerce_product_id = %s AND bigcommerce_product_attribute_id = %s AND bigcommerce_store_id=%s AND company_id in %s"""
            self.env.cr.execute(product_query, (prod['product_id'], prod['variant_id'],self.id))
            ...
            if not product:
                ...
                product = self.env.cr.fetchall()
                _logger.info(f'>>product1 {product}')
            # if not product:
            #     _logger.warning("Product %s (variant %s) not found for order %s, skipping line",
            #                     prod['product_id'], prod['variant_id'], order_data['id'])
            #     continue

            product_ids = [row[0] for row in product]
            product = self.env['product.product'].browse(product_ids[0]) if product_ids else None
        ...
        if not order_lines:
            error_msg = f"No valid order lines for order {order_data['id']}"
            self.env['cr.data.processing.log']._log_data_processing(
                cr_shop_id=self.id,
                record_count=1,
                cr_message="Failed to process order: No valid order lines",
                status='failure',
                timespan=str(fields.Datetime.now()),
                initiated_at=str(fields.Datetime.now()),
                error_message=error_msg,
            ...
            _logger.warning("No valid order lines for order %s, skipping", order_data['id'])
            return False
        ...
        if isinstance(shipping_addresses_data, dict) and 'url' in shipping_addresses_data:
            try:
                ...
                _logger.error("Failed to fetch shipping addresses for order %s: %s", order_data['id'], str(e))

        shipping_cost = float(order_data.get('shipping_cost_inc_tax', 0))
        carrier = False
        location = False
        tracking_number = False
        shipped_items = []
        ...
        if shipping_addresses and len(shipping_addresses) > 0:
            ...
            if location_id:
                ...
                self.env.cr.execute(location_query, (location_id, self.id,(False, self.company_id.id)))
                location = self.env.cr.fetchall()
                _logger.info(f'>>location {location}')
        else:
            _logger.info("No shipping addresses for order %s, proceeding without zone or location", order_data['id'])


        # Update status based on BigCommerce status
        bc_status = order_data.get('status')
        _logger.info(f'bc_status {bc_status}')
        # if bc_status == 'Cancelled' and sale_order.state != 'cancel':
        #     sale_order._action_cancel()
        # elif bc_status in ['Awaiting Fulfillment', 'Awaiting Shipment'] and sale_order.state == 'draft':
        #     sale_order.action_confirm()
        ...
        if order_data.get('status') == 'Shipped':
            ...
            try:
                shipment_response = requests.get(shipment_url, headers=headers)
                _logger.info("shipment_response %s for order %s", shipment_response.status_code, order_data['id'])
                if shipment_response.status_code == 204:
                    _logger.info("No shipment data for order %s (204 No Content)", order_data['id'])
                else:
                    shipment_response.raise_for_status()
                    if shipment_response.text:
                        try:
                            ...
                            if isinstance(shipments, list) and shipments:
                                ...
                                # print("shipment['shipping_method'] : ", shipment['shipping_method'])
                                # print("t : ", type(shipment['shipping_method']))
                                # if shipment['shipping_method'] not in [None, '', 'None']:
                                #     carrier_query = """
                                #         SELECT * FROM delivery_carrier
                                #         WHERE  store_id = %s
                                #           AND name::text ILIKE %s
                                #     """
                                #     self.env.cr.execute(carrier_query, (self.id, shipment['shipping_method']))
                                #     carrier = self.env.cr.fetchall()
                                #     _logger.info(f'>>carrier {carrier}')
                                if not carrier:
                                    carrier = self.env['delivery.carrier'].sudo().search([
                                        ('delivery_type', '=', 'bigcommerce'),
                                        ('store_id', '=', self.id),
                                    ...
                                    # carrier_query2 = f"""SELECT * FROM delivery_carrier WHERE  store_id=%s AND name ilike %s"""
                                    # self.env.cr.execute(carrier_query2, (self.id, shipment['shipping_provider_display_name']))
                                    # carrier = self.env.cr.fetchall()
                                    # _logger.info(f'>>carrier2 {carrier}')

                                tracking_number = shipment.get('tracking_number')
                                shipped_items = shipment.get('items', [])
                            else:
                                _logger.info("No shipments found for order %s", order_data['id'])
                            ...
                            _logger.error("Invalid JSON response for shipments of order %s: %s. Response text: %s",
                                          order_data['id'], str(e), shipment_response.text)
                    else:
                        _logger.info("Empty response for shipments of order %s", order_data['id'])
            except requests.exceptions.RequestException as e:
                error_msg = f"Failed to fetch shipments for order {order_data['id']}: {str(e)}"
                self.env['cr.data.processing.log']._log_data_processing(
                    cr_shop_id=self.id,
                    record_count=1,
        ...
        if carrier and shipping_cost > 0:
            ...
            if carrier.minimum_sub_total and subtotal < carrier.minimum_sub_total:
                _logger.warning("Order %s subtotal (%s) below minimum (%s), skipping shipping",
                                order_data['id'], subtotal, carrier.minimum_sub_total)
                final_shipping_cost = 0
            else:
                final_shipping_cost += carrier.fixed_surcharge
        ...
        for row in status_result:
            status_result_dict = dict(zip(columns, row))
            status_result_data = status_result_dict
            _logger.info(f">> status_result_dict: {status_result_dict}")

        # Determine Odoo state
        odoo_state = 'sale' if order_data.get('status') in ['Shipped', 'Completed'] else 'draft'

        # Parse date
        try:
            date_order = datetime.strptime(order_data['date_created'], '%a, %d %b %Y %H:%M:%S %z').replace(tzinfo=None)
            # date_order = datetime.strptime(order_data['date_created'], '%a, %d %b %Y %H:%M:%S %z').replace(tzinfo=None)
        except ValueError:
            date_order = fields.Datetime.now()
            _logger.warning("Invalid date format for order %s, using current time", order_data['id'])

        currency = order_data.get('currency_id', '')
        print('currency  : ',currency)
        # currency_id = self.env['res.currency'].search([('bc_currency_id','=',currency)])
        ...
        # currency_id = self.env.cr.fetchall()
        _logger.info(f'>>currency_data {currency_data}')
        ...
        sale_order_vals = {
            ...
            'bigcommerce_store_id': self.id,
            'bigcommerce_order_id': order_data['id'],
            'bigcommerce_order_status_id': status_result_data.get('id') if status_result_data else False,
            'bigcommerce_subtotal_ex_tax': float(order_data.get('subtotal_ex_tax', 0)),
            'bigcommerce_subtotal_inc_tax': float(order_data.get('subtotal_inc_tax', 0)),
            'bigcommerce_subtotal_tax': float(order_data.get('subtotal_tax', 0)),
            'bigcommerce_base_shipping_cost': float(order_data.get('base_shipping_cost', 0)),
            'bigcommerce_shipping_cost_ex_tax': float(order_data.get('shipping_cost_ex_tax', 0)),
            'bigcommerce_shipping_cost_inc_tax': float(order_data.get('shipping_cost_inc_tax', 0)),
            ...
            'carrier_id': carrier.id if carrier else False,
            'company_id': self.company_id.id,
            'currency_id': currency_data.get('id') if currency_data else self.company_id.currency_id.id,  # fallback if not found
        }

        # Create or update sale order
        if sale_order:
            sale_order.write(sale_order_vals)
            _logger.info("Updated sale order for BigCommerce order %s", order_data['id'])
        else:
            if self.order_sequence:
                ...
                sequence_query = f"""SELECT * FROM ir_sequence WHERE code = %s"""
                self.env.cr.execute(sequence_query, (sequence_code,))
                sequence = self.env.cr.fetchall()
                _logger.info(f'>>sequence {sequence}')
            ...
            sale_order = self.env['sale.order'].sudo().create(sale_order_vals)
            # sale_order.currency_id = currency_id.id if currency_id else self.company_id.currency_id.id
            # sale_order.company_id = self.company_id.id
        ...
        if carrier and final_shipping_cost > 0:
            if sale_order._set_delivery_cost(carrier, final_shipping_cost):
                _logger.info("Set delivery cost %s for order %s with carrier %s",
                             final_shipping_cost, order_data['id'], carrier.name)
            else:
        ...
        # Handle shipped orders (status_id == 10)
        if order_data.get('status') == 'Shipped':
            picking = sale_order.picking_ids.filtered(lambda p: p.state not in ['done', 'cancel'])
            if not picking:
                sale_order._action_create_delivery()
                picking = sale_order.picking_ids.filtered(lambda p: p.state not in ['done', 'cancel'])
            ...
            if picking:
                ...
                if shipped_items:
                    for item in shipped_items:
                        ...
                        #     ('bigcommerce_product_id', '=', item['product_id']),
                        # ], limit=1)
                        product_query = f"""SELECT * FROM product_product WHERE bigcommerce_product_id = %s AND bigcommerce_store_id = %s"""
                        self.env.cr.execute(product_query, (item['product_id'],self.id))
                        ...
                        if product_data:
                            move = picking.move_ids_without_package.filtered(
                                lambda m: m.product_id.id == product_data.get('id'))
                            if move:
                                move.quantity = item['quantity']
                                # _logger.info("move.quantity %s %s", move.quantity, move)
                else:
                    _logger.info("No shipped items for order %s, skipping picking validation", order_data['id'])

                if shipped_items and any(move.quantity > 0 for move in picking.move_ids_without_package):
                    try:
                        picking.action_assign()
                        picking.button_validate()
                        log_message = f"Created and validated delivery for order {order_data['id']} from location {location.name if location else 'default'} with tracking {tracking_number or 'none'}"
                        _logger.info(log_message)
                        self.env['cr.data.processing.log']._log_data_processing(
                            cr_shop_id=self.id,
                            record_count=1,
    ...
    def action_import_bigcommerce_orders(self):
        ...
        for store in self:
            ...
            for order in orders:
                # data = self.env['sale.order'].sudo().search([('bigcommerce_order_id', '=', order['id'])],
                #                                             limit=1)
                print(' : ',order['id'])
                print(' store.id ',store.id)
                # query = f"""SELECT * FROM sale_order WHERE bigcommerce_order_id = %s AND bigcommerce_store_id = %s"""
                # self.env.cr.execute(query, (order['id'], store.id))
                ...
                if data:
                    ...
                    if self._process_bigcommerce_order(order, headers):
                        imported_count += 1

                # Log completion of import
                self.env['cr.data.processing.log']._log_data_processing(
                    cr_shop_id=store.id,
                    record_count=imported_count,
                    cr_message=f"Successfully imported {imported_count} sale order(s) for store {store.store_hash}",
                    status='success',
                    timespan=str(fields.Datetime.now()),
                    initiated_at=str(fields.Datetime.now()),
                    error_message='',
    ...
    def _fetch_product_lines(self, products_url, headers, order_id):
        ...
        for prod in prod_response.json():
            ...
            if not product:
                ...
                # _logger.warning("Product %s (variant %s) not found for order %s, skipping line",
                #                 prod['product_id'], prod['variant_id'], order_id)
                # continue
    ...
    def _fetch_shipping_details(self, order_data, headers, order_id):
        ...
        if isinstance(shipping_addresses_data, dict) and 'url' in shipping_addresses_data:
            try:
                ...
                error_msg = f"Failed to fetch shipping addresses for order {order_id}: {str(e)}"
                self.env['cr.data.processing.log']._log_data_processing(
                    cr_shop_id=self.id,
                    record_count=1,
        ...
        if shipping_addresses and len(shipping_addresses) > 0:
            ...
            _logger.info("No shipping addresses for order %s, proceeding without zone or location", order_id)
        ...
        if order_data.get('status') == 'Shipped':
            ...
            try:
                shipment_response = requests.get(shipment_url, headers=headers)
                _logger.info("shipment_response %s for order %s", shipment_response.status_code, order_id)
                if shipment_response.status_code == 204:
                    self.env['cr.data.processing.log']._log_data_processing(
                        cr_shop_id=self.id,
                        record_count=1,
                    ...
                    _logger.info("No shipment data for order %s (204 No Content)", order_id)
                else:
                    shipment_response.raise_for_status()
                    if shipment_response.text:
                        try:
                            ...
                            if isinstance(shipments, list) and shipments:
                                ...
                                _logger.info("No shipments found for order %s", order_id)
                        except ValueError as e:
                        ...
                        _logger.info("Empty response for shipments of order %s", order_id)
            except requests.exceptions.RequestException as e:
        ...
        if carrier and shipping_cost > 0:
            ...
            if carrier.minimum_sub_total and subtotal < carrier.minimum_sub_total:
                _logger.warning("Order %s subtotal (%s) below minimum (%s), skipping shipping",
                                order_id, subtotal, carrier.minimum_sub_total)
                final_shipping_cost = 0
            else:
                final_shipping_cost += carrier.fixed_surcharge
        ...
        if carrier:
            ...
            shipping_tax = self.env['account.tax'].sudo().search([
    ...
    # def _prepare_sale_order_vals(self, order_data, customer, status, odoo_state, carrier):
    ...
    def _update_bigcommerce_order(self, order_data, headers, sale_order):
        ...
        if not customer:
            self.env['cr.data.processing.log']._log_data_processing(
                ...
                timespan=str(fields.Datetime.now()),
                initiated_at=str(fields.Datetime.now()),
                error_message=f"Customer {order_data['customer_id']} not found for order {order_id}",
                cr_user_id=self.env.uid
            )
            return False
        ...
        if not order_lines:
            self.env['cr.data.processing.log']._log_data_processing(
                ...
                timespan=str(fields.Datetime.now()),
                initiated_at=str(fields.Datetime.now()),
                error_message=f"No valid order lines for order {order_id}",
                cr_user_id=self.env.uid
            )
            return False
        ...
        # Prepare sale order values
        sale_order_vals = self._prepare_sale_order_vals(order_data, customer, status, odoo_state, carrier)

        # Update order lines
        existing_lines = sale_order.order_line.filtered(lambda l: not l.is_delivery)
        updated_lines = []
        matched_line_ids = []

        for line_data in order_lines:
            matching_line = existing_lines.filtered(
                lambda l: l.product_id.bigcommerce_product_id == line_data['bigcommerce_product_id'] and
                          l.product_id.bigcommerce_product_attribute_id == line_data['bigcommerce_product_attribute_id']
            )
            if matching_line:
                # Update existing line
                updated_lines.append((1, matching_line[0].id, {
                    'product_uom_qty': line_data['product_uom_qty'],
                    'price_unit': line_data['price_unit'],
                    'tax_id': line_data['tax_id'],
                }))
                matched_line_ids.append(matching_line[0].id)
            else:
                # Add new line
        ...
        # Remove lines not in BigCommerce order
        for line in existing_lines:
            if line.id not in matched_line_ids:
                updated_lines.append((2, line.id, 0))
                _logger.info("Removed sale order line %s for order %s", line.id, order_id)

        sale_order_vals['order_line'] = updated_lines
        ...
        try:
            sale_order.write(sale_order_vals)
            _logger.info("Updated sale order for BigCommerce order %s", order_id)
            self.env['cr.data.processing.log']._log_data_processing(
                cr_shop_id=self.id,
                record_count=1,
        ...
        if carrier and final_shipping_cost > 0:
            if sale_order._set_delivery_cost(carrier, final_shipping_cost):
                self.env['cr.data.processing.log']._log_data_processing(
                    cr_shop_id=self.id,
                    record_count=1,
                    cr_message=f"Set delivery cost {final_shipping_cost} for order {order_id} with carrier {carrier.name}",
                    status='success',
                    timespan=str(fields.Datetime.now()),
                    initiated_at=str(fields.Datetime.now()),
                    error_message='',
                ...
                _logger.info("Set delivery cost %s for order %s with carrier %s",
                             final_shipping_cost, order_id, carrier.name)
            else:
                error_msg = f"Failed to set delivery cost for order {order_id}: No delivery product for carrier {carrier.name}"
                self.env['cr.data.processing.log']._log_data_processing(
                    cr_shop_id=self.id,
                    record_count=1,
                    cr_message=f"Failed to set delivery cost for order {order_id}",
                    status='failure',
                    timespan=str(fields.Datetime.now()),
                    initiated_at=str(fields.Datetime.now()),
                    error_message=error_msg,
                ...
                if shipped_items:
                    for item in shipped_items:
                        ...
                        if product:
                            ...
                            if move:
                                move.quantity = item['quantity']
                                _logger.info("move.quantity_done %s %s", move.quantity, move)
                else:
                    self.env['cr.data.processing.log']._log_data_processing(
                        cr_shop_id=self.id,
                        record_count=1,
                        cr_message=f"No shipped items for order {order_id}, skipping picking validation",
                        status='success',
                        timespan=str(fields.Datetime.now()),
                        initiated_at=str(fields.Datetime.now()),
                        error_message='',
                    ...
                    _logger.info("No shipped items for order %s, skipping picking validation", order_id)
                ...
                if shipped_items and any(move.quantity > 0 for move in picking.move_ids_without_package):
                    try:
                        ...
                        self.env['cr.data.processing.log']._log_data_processing(
                            cr_shop_id=self.id,
                            record_count=1,
                            cr_message=f"Created and validated delivery for order {order_id} from location {location.name if location else 'default'} with tracking {tracking_number or 'none'}",
                            status='success',
                            timespan=str(fields.Datetime.now()),
                            initiated_at=str(fields.Datetime.now()),
                            error_message='',
                        ...
                        _logger.info("Created and validated delivery for order %s from location %s with tracking %s",
                                     order_id, location.name if location else 'default',
                                     tracking_number or 'none')
                    except Exception as e:
                        error_msg = f"Failed to validate delivery for order {order_id}: {str(e)}"
                        self.env['cr.data.processing.log']._log_data_processing(
                            cr_shop_id=self.id,
                            record_count=1,
                            cr_message=f"Failed to validate delivery for order {order_id}",
                            status='failure',
                            timespan=str(fields.Datetime.now()),
                            initiated_at=str(fields.Datetime.now()),
                            error_message=error_msg,
                    ...
                    error_msg = f"No quantities reserved or set for delivery of order {order_id}"
                    self.env['cr.data.processing.log']._log_data_processing(
                        cr_shop_id=self.id,
                        record_count=1,
                        cr_message=f"Failed to validate delivery for order {order_id}: No quantities reserved",
                        status='failure',
                        timespan=str(fields.Datetime.now()),
                        initiated_at=str(fields.Datetime.now()),
                        error_message=error_msg,
    ...
    # def _sync_tax_classes(self):
    #     """Sync BigCommerce tax classes to Odoo taxes."""
    #     headers = {
    #         "X-Auth-Token": self.access_token,
    #         "Accept": "application/json",
    #         "Content-Type": "application/json"
    #     }
    #     url = f"https://api.bigcommerce.com/stores/{self.store_hash}/v2/tax_classes"
    #     try:
    #         response = requests.get(url, headers=headers)
    #         response.raise_for_status()
    #         tax_classes = response.json()
    #     except requests.exceptions.RequestException as e:
    #         _logger.error("Failed to fetch tax classes for store %s: %s", self.store_hash, str(e))
    #         raise UserError(_("Failed to fetch tax classes: %s") % str(e))
    #
    #     for tc in tax_classes:
    #         tax_name = f"Tax {tc['name']}"
    #         tax = self.env['account.tax'].sudo().search([
    #             ('bigcommerce_tax_class_id', '=', tc['id']),
    #             ('company_id', '=', self.env.company.id)
    #         ], limit=1)
    #         if not tax:
    ...
    #                 'type_tax_use': 'sale',
    #                 'amount_type': 'percent',
    #                 'amount': 0,  # Updated via rates
    #                 'company_id': self.env.company.id,
    #                 'bigcommerce_tax_class_id': tc['id'],
    #                 'price_include': True,  # Default to inclusive per API data
    #             })
    #             _logger.info("Created tax %s for BigCommerce tax class %s", tax_name, tc['id'])
    #
    #     return True
    #
    ...
    #     except requests.exceptions.RequestException as e:
    #         _logger.error("Failed to fetch tax zones for store %s: %s", self.store_hash, str(e))
    #         raise UserError(_("Failed to fetch tax zones: %s") % str(e))
    #
    #     # Fetch all tax rates
    ...
    #     except requests.exceptions.RequestException as e:
    #         _logger.error("Failed to fetch tax rates for store %s: %s. Response: %s",
    #                       self.store_hash, str(e), getattr(rates_response, 'text', 'No response'))
    #         raise UserError(_("Failed to fetch tax rates: %s") % str(e))
    #
    #     # Group rates by tax_zone_id
    ...
    #         country_id = False
    #         state_ids = []
    #         for loc in zone['shopper_target_settings']['locations']:
    #             if loc.get('country_code'):
    #                 country = self.env['res.country'].search([('code', '=', loc['country_code'])], limit=1)
    #                 if country:
    #                     country_id = country.id
    #             if loc.get('subdivision_codes'):
    #                 states = self.env['res.country.state'].search([('code', 'in', loc['subdivision_codes'])])
    #                 if states:
    #                     state_ids.extend(states.ids)
    #
    #         # Process tax rates for this zone
    #         tax_mappings = []
    #         zone_rates = rates_by_zone.get(zone_id, [])
    #         if not zone_rates:
    #             _logger.warning("No tax rates found for zone %s (%s), using 0%% rates", zone_id, zone_name)
    #             skipped_zones.append(f"{zone_name} (ID: {zone_id})")
    #             # Assign 0% rates to all tax classes
    ...
    #                     tax = self.env['account.tax'].sudo().search([
    #                         ('bigcommerce_tax_class_id', '=', str(class_rate['tax_class_id'])),
    #                         ('company_id', '=', self.env.company.id)
    #                     ], limit=1)
    #                     if tax:
    #                         tax.write({'amount': class_rate['rate']})
    #                         tax_mappings.append({
    #                             'tax_src_id': tax.id,
    #                             'tax_dest_id': tax.id,
    #                         })
    #
    #         # Create or update fiscal position
    #         fp_vals = {
    #             'name': zone_name,
    #             'company_id': self.env.company.id,
    #             'bigcommerce_zone_id': zone_id,
    ...
    #             'state_ids': [(6, 0, state_ids)] if state_ids else [(5, 0, 0)],
    #         }
    #         if not fiscal_position:
    #             fiscal_position = self.env['account.fiscal.position'].sudo().create(fp_vals)
    #             _logger.info("Created fiscal position %s for BigCommerce zone %s", zone_name, zone_id)
    #         else:
    #             fiscal_position.write(fp_vals)
    #
    #     return {'success': True, 'skipped_zones': skipped_zones}
    #
    ...
    #     except requests.exceptions.RequestException as e:
    #         _logger.error("Failed to fetch products for store %s: %s", self.store_hash, str(e))
    ...
    #             _logger.info("Updated product %s with tax class %s", prod['id'], tax_class_id)
    #         else:
    #             product_vals['type'] = 'product'
    #             product = self.env['product.product'].sudo().create(product_vals)
    #             _logger.info("Created product %s with tax class %s", prod['id'], tax_class_id)
    #
    #     return True
    #
    ...
    #     try:
    #         # Sync tax classes
    #         if not self._sync_tax_classes():
    #             raise UserError(_("Failed to sync tax classes for store %s.") % self.store_hash)
    #
    #         # Sync tax zones and rates
    #         zone_result = self._sync_tax_zones_and_rates()
    #         if not zone_result['success']:
    #             raise UserError(_("Failed to sync tax zones and rates for store %s.") % self.store_hash)
    #         skipped_zones = zone_result['skipped_zones']
    #
    #         # Sync products to assign taxes
    #         if not self._sync_products():
    #             raise UserError(_("Failed to sync products for store %s.") % self.store_hash)
    #
    #         # Prepare notification message
    #         message = _("Taxes and products synced successfully for store %s.") % self.store_hash
    #         if skipped_zones:
    #             message += _(" Skipped zones due to missing rates: %s.") % ", ".join(skipped_zones)
    #
    ...
    #         }
    #     except Exception as e:
    #         _logger.error("Error syncing taxes and products for store %s: %s", self.store_hash, str(e))
    ...
    def _sync_tax_classes(self):
        ...
        try:
            ...
            self.env['cr.data.processing.log']._log_data_processing(
                cr_shop_id=self.id,
                record_count=1,
                cr_message=f"Fetched {len(tax_classes)} tax classes for store {self.store_hash}",
                status='success',
                timespan=str(fields.Datetime.now()),
                initiated_at=str(fields.Datetime.now()),
                error_message='',
            ...
            error_msg = f"Failed to fetch tax classes for store {self.store_hash}: {str(e)}"
            _logger.error(error_msg)
            self.env['cr.data.processing.log']._log_data_processing(
                cr_shop_id=self.id,
                record_count=1,
                cr_message="Failed to fetch tax classes",
                status='failure',
                timespan=str(fields.Datetime.now()),
                initiated_at=str(fields.Datetime.now()),
                error_message=error_msg,
        ...
        for tc in tax_classes:
            ...
            if not tax:
                ...
                self.env['cr.data.processing.log']._log_data_processing(
                    cr_shop_id = self.id,
                    record_count = 1,
                    cr_message = f"Created tax {tax_name} for BigCommerce tax class {tc['id']} (store {self.store_hash})",
                    status = 'success',
                    timespan = str(fields.Datetime.now()),
                    initiated_at = str(fields.Datetime.now()),
                    error_message = '',
                ...
                _logger.info("Created tax %s for BigCommerce tax class %s (store %s)", tax_name, tc['id'],
                             self.store_hash)
    ...
    def _sync_tax_zones_and_rates(self):
        ...
        try:
            ...
            self.env['cr.data.processing.log']._log_data_processing(
                cr_shop_id=self.id,
                record_count=1,
                cr_message=f"Fetched {len(zones)} tax zones for store {self.store_hash}",
                status='success',
                timespan=str(fields.Datetime.now()),
                initiated_at=str(fields.Datetime.now()),
                error_message='',
            ...
            error_msg = f"Failed to fetch tax zones for store {self.store_hash}: {str(e)}"
            _logger.error(error_msg)
            self.env['cr.data.processing.log']._log_data_processing(
                cr_shop_id=self.id,
                record_count=1,
                cr_message="Failed to fetch tax zones",
                status='failure',
                timespan=str(fields.Datetime.now()),
                initiated_at=str(fields.Datetime.now()),
                error_message=error_msg,
        ...
        try:
            ...
            self.env['cr.data.processing.log']._log_data_processing(
                cr_shop_id=self.id,
                record_count=1,
                cr_message=f"Fetched {len(rates_data)} tax rates for store {self.store_hash}",
                status='success',
                timespan=str(fields.Datetime.now()),
                initiated_at=str(fields.Datetime.now()),
                error_message='',
            ...
            error_msg = f"Failed to fetch tax rates for store {self.store_hash}: {str(e)}"
            _logger.error(error_msg)
            self.env['cr.data.processing.log']._log_data_processing(
                cr_shop_id=self.id,
                record_count=1,
                cr_message="Failed to fetch tax rates",
                status='failure',
                timespan=str(fields.Datetime.now()),
                initiated_at=str(fields.Datetime.now()),
                error_message=error_msg,
        ...
        for zone in zones:
            ...
            if not zone_rates:
                self.env['cr.data.processing.log']._log_data_processing(
                    cr_shop_id=self.id,
                    record_count=1,
                    cr_message=f"No tax rates found for zone {zone_name} (ID: {zone_id}), using 0% rates (store {self.store_hash})",
                    status='success',
                    timespan=str(fields.Datetime.now()),
                    initiated_at=str(fields.Datetime.now()),
                    error_message='',
                ...
                _logger.warning("No tax rates found for zone %s (%s), using 0%% rates (store %s)",
                                zone_id, zone_name, self.store_hash)
                skipped_zones.append(f"{zone_name} (ID: {zone_id})")
                for tax in self.env['account.tax'].sudo().search([
                    ('bigcommerce_tax_class_id', 'in', ['0', '1', '2', '3']),
                    ('bigcommerce_store_id', '=', self.id),
            ...
            if not fiscal_position:
                ...
                self.env['cr.data.processing.log']._log_data_processing(
                    cr_shop_id=self.id,
                    record_count=1,
                    cr_message=f"Created fiscal position {zone_name} for BigCommerce zone {zone_id} (store {self.store_hash})",
                    status='success',
                    timespan=str(fields.Datetime.now()),
                    initiated_at=str(fields.Datetime.now()),
                    error_message='',
                ...
                created_positions += 1
                _logger.info("Created fiscal position %s for BigCommerce zone %s (store %s)",
                             zone_name, zone_id, self.store_hash)
            else:
                fiscal_position.write(fp_vals)
                self.env['cr.data.processing.log']._log_data_processing(
                    cr_shop_id=self.id,
                    record_count=1,
                    cr_message=f"Updated fiscal position {zone_name} for BigCommerce zone {zone_id} (store {self.store_hash})",
                    status='success',
                    timespan=str(fields.Datetime.now()),
                    initiated_at=str(fields.Datetime.now()),
                    error_message='',
            ...
            # Log completion of tax zones and rates synchronization
            self.env['cr.data.processing.log']._log_data_processing(
                cr_shop_id=self.id,
                record_count=len(zones),
                cr_message=f"Completed tax zones and rates synchronization for store {self.store_hash}: processed {len(zones)} zones, created {created_positions} fiscal positions, updated {updated_positions} fiscal positions, skipped {len(skipped_zones)} zones",
                status='success',
                timespan=str(fields.Datetime.now()),
                initiated_at=str(fields.Datetime.now()),
                error_message='',
    ...
    def _sync_products(self):
        ...
        try:
            ...
            error_msg = f"Failed to fetch products for store {self.store_hash}: {str(e)}"
            self.env['cr.data.processing.log']._log_data_processing(
                cr_shop_id=self.id,
                record_count=1,
        ...
        for prod in products:
            ...
            if product:
                ...
                updated_products += 1
                _logger.info("Updated product %s with tax class %s (store %s)",
                             prod['id'], tax_class_id, self.store_hash)
            else:
                product_vals['type'] = 'product'
                product = self.env['product.product'].sudo().create(product_vals)
                self.env['cr.data.processing.log']._log_data_processing(
                ...
                created_products += 1
                _logger.info("Created product %s with tax class %s (store %s)",
                             prod['id'], tax_class_id, self.store_hash)
    ...
    def _prepare_sale_order_vals(self, order_data, customer, status, odoo_state, carrier):
        ...
        country = self.env['res.country'].search([('code', '=', shipping_addresses[0].get('country_iso2'))], limit=1)
        _logger.info(f'country {country}')
        state = self.env['res.country.state'].search([('code', '=', shipping_addresses[0].get('state'))], limit=1)
    ...
    def action_sync_taxes(self):
        ...
        try:
            # Sync tax classes
            if not self._sync_tax_classes():
                error_msg = f"Failed to sync tax classes for store {self.store_hash}"
                self.env['cr.data.processing.log']._log_data_processing(
                    cr_shop_id=self.id,
                    record_count=1,
                    cr_message="Failed to sync tax classes",
                    status='failure',
                    timespan=str(fields.Datetime.now()),
                    initiated_at=str(fields.Datetime.now()),
                    error_message=error_msg,
            ...
            self.env['cr.data.processing.log']._log_data_processing(
                cr_shop_id=self.id,
                record_count=1,
                cr_message=f"Synced tax classes for store {self.store_hash}",
                status='success',
                timespan=str(fields.Datetime.now()),
                initiated_at=str(fields.Datetime.now()),
                error_message='',
            ...
            zone_result = self._sync_tax_zones_and_rates()
            if not zone_result['success']:
                error_msg = f"Failed to sync tax zones and rates for store {self.store_hash}"
                _logger.error(error_msg)
                self.env['cr.data.processing.log']._log_data_processing(
                    cr_shop_id=self.id,
                    record_count=1,
            ...
            self.env['cr.data.processing.log']._log_data_processing(
                ...
                timespan=str(fields.Datetime.now()),
                initiated_at=str(fields.Datetime.now()),
                error_message='' if not skipped_zones else f"Skipped zones: {', '.join(skipped_zones)}",
                cr_user_id=self.env.uid
            )

            # Sync products to assign taxes
            if not self._sync_products():
                error_msg = f"Failed to sync products for store {self.store_hash}"
                self.env['cr.data.processing.log']._log_data_processing(
                    cr_shop_id=self.id,
                    record_count=1,
                    cr_message="Failed to sync products",
                    status='failure',
                    timespan=str(fields.Datetime.now()),
                    initiated_at=str(fields.Datetime.now()),
                    error_message=error_msg,
            ...
            self.env['cr.data.processing.log']._log_data_processing(
                cr_shop_id=self.id,
                record_count=1,
                cr_message=f"Synced products for store {self.store_hash}",
                status='success',
                timespan=str(fields.Datetime.now()),
                initiated_at=str(fields.Datetime.now()),
                error_message='',
    ...
    def create_cron_jobs(self):
        ...
        try:
            ...
            if not self.import_scheduled_units:
                error_msg = "Missing import scheduled units for store %s" % self.store_hash
                _logger.error(error_msg)
                self.env['cr.data.processing.log']._log_data_processing(
                    cr_shop_id=self.id,
                    record_count=1,
    ...
    def create_order_cron(self):
        ...
        try:
            ...
            if not self.import_cron_value1:
                error_msg = "Missing import cron value for orders for store %s" % self.store_hash
                _logger.error(error_msg)
                self.env['cr.data.processing.log']._log_data_processing(
                    cr_shop_id=self.id,
                    record_count=1,
            ...
            if not self.import_scheduled_units1:
                error_msg = "Missing import scheduled units for orders for store %s" % self.store_hash
                _logger.error(error_msg)
                self.env['cr.data.processing.log']._log_data_processing(
                    cr_shop_id=self.id,
                    record_count=1,
    ...
    def _create_cron(self, name, model, method_name, cron_unit, cron_value):
        ...
        try:
            ...
            # Validate model
            model_id = self.env["ir.model"].sudo().search([("model", "=", model)], limit=1).id
            if not model_id:
            ...
            existing_cron = self.env["ir.cron"].sudo().search([("name", "=", name)], limit=1)
            if existing_cron:
                existing_cron.write(
                    {
    ...
    #                                 identity['product_id'], identity['variant_id'], self.store_hash, identity['sku'])
    ...
    def bulk_export_inventory(self):
        products = self.env['product.product'].search([('type', '=', 'product')], limit=50)
