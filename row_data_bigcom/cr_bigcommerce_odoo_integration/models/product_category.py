...
class ProductCategory(models.Model):
    ...
    def action_delete_category_in_bigcommerce(self):
        ...
        url = f"https://api.bigcommerce.com/stores/{store.store_hash}/v3/catalog/trees/categories"
        params = {
            "category_id:in": self.bigcommerce_category_id
        }
        headers = {
            "X-Auth-Token": store.access_token,
            "Accept": "application/json"
        ...
        if response.status_code != 202:
            raise UserError(f"Failed to delete category from BigCommerce: {response.text}")
    ...
    def action_bulk_update_categories_in_bigcommerce(self):
        for record in self:
            try:
                ...
                _logger.error("Failed to update BigCommerce category for record %s: %s", record.id, str(e))
