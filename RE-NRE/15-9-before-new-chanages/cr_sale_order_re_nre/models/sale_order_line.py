...
class SaleOrderLine(models.Model):
    ...
    #     'discount', 'tax_id', 'currency_id',
    #     'nre_sub_line_ids.invoiced_amount', 're_nre'
    # )
    # def _compute_remaining_invoiced_amount(self):
    #     """
    #     Computes remaining invoiced amount:
    #     - NRE lines → sum of sub-lines minus already invoiced (tax-inclusive)
    #     - RE lines → remaining qty × unit price (tax-inclusive)
    #     - Service / other lines → remaining amount based on actual invoices
    #     """
    #     for line in self:
    #         remaining_total = 0.0
    #
    #         if line.re_nre == 'nre' and line.nre_sub_line_ids:
    #             # --- NRE case: sum of sub-lines including tax from parent sale line ---
    #             total_expected = 0.0
    #             for sub in line.nre_sub_line_ids:
    #                 price = sub.billing_amount or 0.0
    ...
    #                 )
    #                 total_expected += taxes_res['total_included']
    #
    #             total_invoiced = sum(sub.invoiced_amount or 0.0 for sub in line.nre_sub_line_ids)
    #             remaining_total = max(total_expected - total_invoiced, 0.0)
    #
    #         else:
    #             # --- RE / Service / Other lines ---
    ...
    #                     partner=line.order_id.partner_id
    #                 )
    #                 remaining_total = taxes_res['total_included']
    #
    ...
    def _compute_remaining_invoiced_amount(self):
        """
        Computes remaining invoiced amount (without tax logic):
        - NRE lines → sum of sub-lines billing amount minus already invoiced
        - RE lines → remaining qty × unit price (discount applied)
        - Service / other lines → remaining amount based on actual invoices
        """
        for line in self:
            remaining_total = 0.0
            ...
            if line.re_nre == 'nre' and line.nre_sub_line_ids:
                # --- NRE case: sum of sub-lines (no tax) ---
                total_expected = sum(
                    (sub.billing_amount or 0.0) * getattr(sub, 'quantity', 1.0)
                    for sub in line.nre_sub_line_ids
                )
                total_invoiced = sum(sub.invoiced_amount or 0.0 for sub in line.nre_sub_line_ids)
                remaining_total = max(total_expected - total_invoiced, 0.0)
    ...
    def _compute_everest_pn(self):
        """Generate Everest PN code in format EVR + SO number + qty + row."""
        for line in self:
    ...
    #     """Compute remaining quantity to deliver for each line."""
    #     for line in self:
    ...
    def _compute_remaining_qty(self):
        ...
        for line in self:
            if line.re_nre == "nre" and line.nre_sub_line_ids:
                ...
                print(f"[DEBUG NRE] Line {line.id}: qty={line.product_uom_qty}, "
                      f"invoiced%={total_percentage}, remaining={line.remaining_qty_to_deliver}")
            else:
                # --- RE or normal line: qty based ---
                line.remaining_qty_to_deliver = line.product_uom_qty - line.qty_delivered
                print(f"[DEBUG RE] Line {line.id}: qty={line.product_uom_qty}, "
                      f"delivered={line.qty_delivered}, remaining={line.remaining_qty_to_deliver}")

    def create(self, vals):
        """SuperCall create to recompute line numbers after record creation."""
    ...
    def action_sync_re_sub_lines(self,delivery_name=None):
        ...
        # # 🔹 Case: NRE
        # if self.re_nre == 'nre':
        #     if self.nre_sub_line_ids:
        #         self.nre_sub_line_ids.write({
    ...
    #     """Generate or update RE sub-lines based on quantity and delivery status."""
    #     self.ensure_one()
    #     delivered_qty = int(self.qty_delivered or 0)
    ...
    #     else:
    #         # CASE 2: Update existing sub-lines
    #         for idx, sub_line in enumerate(self.re_sub_line_ids.sorted('id'), start=1):
    #             sub_line_code = f"EVR.{line_number_str}.{str(idx).zfill(2)}"
    ...
    def write(self, vals):
        ...
        if "product_uom_qty" in vals or "re_nre" in vals:
            for line in self:
                if line.re_nre == "re":
                    print(f"[DEBUG] Re-generating sub-lines for SO line {line.id}")
    ...
    def _generate_re_sub_lines(self):
        ...
        print(
            f"[DEBUG] SO Line {self.id}: current_qty={current_qty}, target_qty={target_qty}, delivered_qty={delivered_qty}")
        ...
        if target_qty > current_qty:
            print(f"[DEBUG] Adding {target_qty - current_qty} new sub-lines")
            new_lines = []
            for i in range(current_qty + 1, target_qty + 1):
                ...
                vals = {
                    ...
                    # "required_delivery_date": self.required_delivery_date,
                    # "updated_delivery_date": fields.Date.today() if i <= delivered_qty else False,
                }
                # print(f"[DEBUG] Creating sub-line {sub_line_code} with PN {sub_line_everest_pn}")
        ...
        # 🔹 Case B: Decrease quantity → remove extra sub-lines
            ...
            print(f"[DEBUG] Removing {len(extra_sub_lines)} extra sub-lines")
            if any(sl.qty_delivered > 0 for sl in extra_sub_lines):
                print("[DEBUG] Cannot remove sub-lines, some already delivered")
                raise ValidationError(
                    f"You cannot reduce quantity below {current_qty} "
        ...
        print(f"[DEBUG] Updating all sub-lines to match target_qty={target_qty}")
        for idx, sub_line in enumerate(self.re_sub_line_ids.sorted('id'), start=1):
            # sub_line_code = f"EVR.{line_number_str}.{str(idx).zfill(2)}"
            is_delivered = idx <= delivered_qty
            vals = {
                ...
                "qty_delivered": 1.0 if is_delivered else 0.0,
                "remaining_qty_to_deliver": 0.0 if is_delivered else 1.0,
                # "updated_delivery_date": fields.Date.today() if is_delivered else False,
            }
            # print(f"[DEBUG] Updating sub-line {sub_line.id} → code={sub_line_code}, delivered={is_delivered}")
            sub_line.write(vals)